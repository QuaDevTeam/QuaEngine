...
function text(id: string, value: string, rect: Node['bounds'], fontSize: number, lineHeight: number, color = ink): Node {
  return {
    id,
    kind: 'Text',
    bounds: rect,
...
export function layoutDemoBacklog(root: Node, entries: readonly BacklogEntry[], width: number, height: number): Node {
  ...
  collect(root)
  const px = (width - 1460) / 2
  const py = (height - 930) / 2
  const left = px + 67
  const top = py + 41
  const contentWidth = 1326
  const scrollY = top + 83
  const scrollHeight = 709
  let y = scrollY + 30
  const rows = entries.map((entry, index) => {
    ...
    const font = isChoice ? 25 : 27
    const lineHeight = isChoice ? 45 : 51.3
    ...
    if (isChoice)
      y += 18
    const x = left + 8
    const bodyX = x + (speaker ? 188 : 0) + (isChoice ? 28 : 0)
    ...
    if (speaker)
      children.push({ ...text(`${id}-speaker`, speaker, bounds(x, bodyY + 3, 156, 44.4), 24, 44.4, sea), style: { ...text('', '', bounds(0, 0, 0, 0), 24, 44.4, sea).style, fontWeight: 500 } })
    const jump = original?.children?.find(n => n.id.endsWith('-jump'))
    const voice = original?.children?.find(n => n.id.endsWith('-voice'))
    if (voice)
      children.push({ ...voice, id: `${id}-voice`, bounds: bounds(x + 1182, y + h - 44, 106, 44), style: { color: sea, fontSize: 18, textAlign: 'center' }, text: '重听' })
    const row: Node = { ...original, id, kind: 'Panel', bounds: bounds(x, y, 1288, h), visible: true, style: isChoice ? { backgroundColor: '#e3ebe25e', borderLeftWidth: 2, borderLeftColor: sea } : {}, children, ...(jump ? { intent: jump.intent, interactive: true } : {}) }
    y += h + (isChoice ? 18 : 0)
    return row
  })
  ...
  const button = (id: string, label: string, x: number, edge: string): Node => ({
    id,
    kind: 'Button',
    bounds: bounds(x, footerY + 19, 122, 52),
    text: label,
    visible: true,
  ...
  return { ...root, children: [
    ...
    { ...nodes.get('backlog-panel')!, bounds: bounds(px, py, 1460, 930), style: { backgroundColor: paper, backgroundImage: { assetType: 'images', assetName: 'ui/paper-grain.png' }, backgroundSize: 'none', backgroundPosition: { x: 0, y: 0 }, borderColor: line, borderWidth: 1, borderRadius: 4, boxShadow: { offsetX: 0, offsetY: 18, blurRadius: 64, spreadRadius: 0, color: '#102e2926', inset: false } }, children: [
      ...
      { ...nodes.get('backlog-scroll')!, bounds: bounds(left, scrollY, contentWidth, scrollHeight), clipChildren: true, scrollOffsetY: Math.max(0, y + 30 - scrollY - scrollHeight), children: [...rows, { id: 'backlog-bottom-padding', kind: 'Box', visible: true, bounds: bounds(left, y, 1, 30) }] },
