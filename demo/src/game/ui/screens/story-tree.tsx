...
export function StoryTreeOverlay({ view }: { view: DemoAppView }) {
  return (
    <Stack class="system-overlay">
      ...
      <Panel id="native-story-tree-panel" x={180} y={98.5} width={1560} height={883}>
        <Text id="native-story-tree-title" class="chapter-title" role="heading" x={216} y={132.5} width={1100} height={66}>章节选择</Text>
        ...
        {view.storyTreeItems.map((item, index) => {
          const x = 216 + Math.floor(index / 5) * 779
          const y = 307.5 + index % 5 * 128
          ...
          return (
            <Panel key={item.id} id={item.id} role="button" class={`chapter-row${item.current ? ' is-current' : ''}`} x={x} y={y} width={709} height={128} onClick={item.disabled ? undefined : ui.open(`chapter:${item.id}`)}>
              <Text id={`chapter-number-${item.id}`} class={`chapter-number${item.disabled ? ' is-locked' : ''}`} x={x + 16} y={y + 47.5} width={94} height={32}>{demoChapterLabel(item.chapter)}</Text>
              <Text id={`chapter-name-${item.id}`} class={item.disabled ? 'chapter-name is-locked' : 'chapter-name'} x={x + 132} y={bodyY} width={529} height={titleHeight}>{item.label}</Text>
