...
import { Backdrop, Button, Column, Panel, Stack, Text, ui } from '@quajs/native-ui'
import { DEMO_GAME_ACTIONS, DEMO_UI_METRICS, demoPanelRect } from '../../ui-presentation'

export function GameMenuOverlay({ view }: { view: DemoAppView }) {
  ...
  return (
    <Stack class="system-overlay">
      ...
      <Panel id="native-game-menu-panel" class="game-menu-panel" {...panel}>
        <Text id="native-game-menu-title" class="panel-title" role="heading" x={panel.x + 53} y={panel.y + 45} width={514} height={54}>{view.t('ui.gameMenu.menu')}</Text>
        ...
        <Button id="native-game-menu-header-close" class="parity-close" ariaLabel="关闭" x={panel.x + panel.width - 105} y={panel.y + 45} width={52} height={52} onClick={ui.close('game-menu')}>×</Button>
        ...
        <Column id="native-game-menu-actions" class="game-menu-actions" x={panel.x + 53} y={panel.y + 196.6} width={594} height={374}>
          {DEMO_GAME_ACTIONS.map(action => (
            <Button
              id={`native-game-menu-${action.id === 'continue' ? 'close' : action.id === 'title' ? 'title-action' : action.id}`}
              key={action.id}
              class="panel-action"
              width={594}
              height={action.id === 'title' ? 62 : 65}
              onClick={action.id === 'continue'
                ? ui.close('game-menu')
                : action.id === 'title'
                  ? ui.open('title-confirm')
                  : ui.open(action.id)}
            >
              {action.label}
            </Button>
