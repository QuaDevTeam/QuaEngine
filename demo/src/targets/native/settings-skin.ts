...
const paper = '#f5f3eb', sheet = '#fffdf7', ink = '#29453f', sea = '#42796e', line = '#c7d2c5'
...
export function layoutDemoSettings(root: Node, width: number, height: number): Node {
  ...
  const collect = (node: Node) => { nodes.set(node.id, node); node.children?.forEach(collect) }
  collect(root)
  const panelWidth = 1360, panelHeight = 796.15
  const px = (width - panelWidth) / 2, py = (height - panelHeight) / 2
  const left = px + 53, top = py + 45, content = panelWidth - 106
  const firstWidth = (content - 48) * 1.12 / 2.12
  const secondWidth = content - 48 - firstWidth, secondX = left + firstWidth + 48
  ...
  const make = (source: Node, bounds: Rect, style: Node['style'], text = source.text, children?: Node[]): Node => ({
    ...source, bounds, style, text, children, stateStyles: source.stateStyles
      ? Object.fromEntries(Object.keys(source.stateStyles).map(key => [key, { bounds, style: { borderColor: sea, backgroundColor: '#e3ebe2' } }])) : undefined,
  })
  const get = (id: string) => {
    const node = nodes.get(id)
    if (!node) throw new Error(`Missing demo settings projection ${id}`)
    return node
  }
  ...
  const field = (key: string, x: number, y: number, w: number, h: number, description?: string): Node => {
    ...
    if (!source) throw new Error(`Missing settings field ${key}`)
    const id = source.id, innerX = x + 16, innerWidth = w - 32
    const children: Node[] = [
      make(get(`${id}-label`), rect(innerX, y + 22, innerWidth, 34.5), textStyle(23, 34.5)),
      divider(`${id}-divider`, rect(x, y + h - 1, w, 1)),
    ]
    if (description) children.push({ id: `${id}-description`, kind: 'Text', bounds: rect(innerX, y + 64.5, innerWidth, 28.05), text: description, visible: true, style: textStyle(17, 28.05, '#61736b'), provenance: source.provenance })
    ...
    if (slider?.control) {
      const format = (label: string) => key === 'textSpeedCps' ? `每秒 ${Math.round(parseFloat(label))} 字` : `${(parseFloat(label) / 1000).toFixed(1)} 秒`
      ...
      const tw = innerWidth - 116 - 26, tx = innerX + (tw - 23) * progress + 11.5, ty = cy + 24
      ...
      children.push({ id: `${id}-output-box`, kind: 'Panel', bounds: rect(innerX + innerWidth - 116, cy, 116, 48), visible: true,
        style: { backgroundColor: sheet, borderColor: line, borderWidth: 1, borderRadius: 3 } })
      ...
      const options = select.control.options.map(o => ({ ...o, label: key === 'skipMode'
        ? (/all|全部/i.test(o.label) ? '全部文字' : '仅已读文字') : key === 'frameRateLimit' ? (Number.isFinite(parseInt(o.label)) ? `${parseInt(o.label)} 帧` : '不限') : o.label }))
      children.push(make({ ...select, control: { ...select.control, options } }, rect(innerX, cy, innerWidth, 60),
        { backgroundColor: sheet, borderColor: '#b8c9bd', borderWidth: 1, borderRadius: 5 }, undefined, [
  ...
  return { ...root, children: [
    ...
    named('settings-panel', rect(px, py, panelWidth, panelHeight), { backgroundColor: paper, backgroundImage: { assetType: 'images', assetName: 'ui/paper-grain.png' }, backgroundSize: 'none', backgroundPosition: { x: 0, y: 0 }, borderColor: line, borderWidth: 1, borderRadius: 4,
      boxShadow: { offsetX: 0, offsetY: 18, blurRadius: 64, spreadRadius: 0, color: '#102e2926', inset: false } }, undefined, [
