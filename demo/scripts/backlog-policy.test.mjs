...
const line = {
  id: 'original',
  kind: 'dialogue',
  point: { sceneId: 'demo', stepId: 'line-1' },
  speaker: 'Mara',
  text: '明天见。',
  rewindable: false,
  voiceReplay: false,
...
it('restoring the same authored line retains its original history entry', () => {
...
it('repeated words at another step, scene, or package remain separate records', () => {
  for (const point of [
...
it('changed speaker, text, choice, voice and package dependencies remain visible', () => {
  for (const change of [
    { speaker: '神代凛' },
    { text: '后天见。' },
    { kind: 'choice', choices: [{ id: 'leave', text: '离开' }] },
    { voice: { assetKey: 'another-voice' } },
    { requiredRuntimePackages: ['new-content'] },
