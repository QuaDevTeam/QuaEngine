...
- UI surface 的绘制和命中测试。
- 当前文本路径对普通 ASCII、常见 UI 标点、normal / italic font style、已解析 native `text-align` / `white-space` / `text-overflow` 子集，且 `text-decoration` 为 `none` / `underline` / `line-through` 的 Text / RichText / Button label 生成确定性的内建 bitmap TextAtlas fallback；real WGPU 路径会采样内建 atlas texture 来绘制这些 fallback glyph 和 solid decoration mask。`pre` / `pre-wrap` 的连续空白会在 bitmap 与 placeholder 路径中保留，`clip` / `ellipsis` 会做确定性裁剪和省略号 marker。unsupported glyph 和更复杂 typography 仍回退到 placeholder geometry。它还不是真实 font shaping、bidi、字体 fallback 或字体资源 rasterization 后端。
