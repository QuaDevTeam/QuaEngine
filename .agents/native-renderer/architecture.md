...
- JSON facade 还要要求已解析 dialogue projection 显式携带 `mode`。Rust DTO builder 可以继续为了内部测试默认 `say`，但 native app / QuickJS bridge JSON 不能把缺失 `mode` 的 dialogue 静默当作 `say`，否则会掩盖 engine / compiler 没有输出完整 projection，并可能改变 narration 与 say 的渲染语义。
