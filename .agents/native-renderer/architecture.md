...
打包到具体 Web / Cocos / Native 项目时，核心插件接线不能借项目模板或启动壳“顺手补齐”。项目模板、startup shell、debug/release shell、installer、updater、smoke runner 和 Runtime QPK 只能消费已经 emit 且 validate 过的 active-target manifest；它们不能自己 import 当前目标 core，也不能携带其他目标 core 后过滤。核心插件只允许由当前目标 resolver 注入一次。
...
项目生成层也必须遵守同一个单向门：Web / Cocos / Native 的 template、startup shell、debug/release shell、installer、updater、smoke runner 都只能读取已经校验的 active-target manifest。它们不能导入任何 target core adapter，也不能把当前 active core 再声明一次。核心插件只能从 packager resolver 进入 post-bundle graph；非 `post-bundle` 的 `projectGraphs` 必须保持平台无关。
...
- 在启用 Cargo feature `native-window` 且设置 `QUA_NATIVE_RENDERER_WINDOW_SMOKE=1` 时，打开一个 `winit` 窗口，创建真实 `wgpu::Surface`，通过 `create_real_wgpu_surface_target` 选择 adapter / device / queue 并配置 surface，然后用 real `WgpuNativeRenderBackend` 渲染已解析 projection JSON 并尝试 present。`native-window` 会启用 `image-decode`，并通过内存 `NativeHostApi` 提供默认 fixture 需要的 `ui/panel.png` / `ui/poster.png` bytes；window smoke 使用 `render_json_frame_with_host_texture_lifecycle_sync` 先完成 JSON facade 校验、frame host cleanup、pending texture discovery、real WGPU decode/upload、成功上传后的 prepared frame 重提交，再按产品 loop 顺序执行 mounted-bundle lifecycle sync。smoke 会处理基础 resize / scale-factor 事件：重新执行 `configure_wgpu_surface_for_native_renderer`，再通过 `resize_target_from_surface_config` 同步 offscreen target，并在 JSON 报告里输出 `resizeCount` 与最后一次 resize 的物理尺寸。smoke 也会把 `winit` cursor / mouse input 转成 native pointer event，且在默认 fixture 上执行一次 deterministic pointer probe，验证已解析 UI 命中测试可以通过 `NativeHostApi` 发出 `NativeRendererIntent`；报告包含 `pointerEventCount`、`pointerDispatchCount`、`pointerIntentEmitCount`、`pointerProbeCount` 和 `pointerLastIntentType`，也包含 `textureLifecycleSyncCount`、`textureLifecycleInitialSyncCount`、`textureLifecycleTrackedPackageCount`、`textureLifecycleReleaseAttemptCount`、`textureLifecycleReleasedPackageCount` 和 `textureLifecycleTextureCleanupErrorCount`。present acquisition 返回 `Lost` 或 `Outdated` 时，smoke 会按当前窗口尺寸重新配置 surface / backend target，记录 `surfaceRecoveryCount`，并请求下一次 redraw。输出 `Qua native window smoke json: ...`，其中 `textureUploadUploadedCount` / `textureUploadResubmitCount` 是本次 smoke 的累计真实上传指标，`textureUploadAlreadyResidentCount` / `textureUploadPendingRequestCount` 是最后一帧的资源状态；`presented=false` / `OccludedAfterRetry` 表示 window/surface/offscreen render 已跑通，但当前 OS 会话没有给出可 present 的可见 swapchain texture，不能当作真实 swapchain present 已验证。这个路径只用于 smoke，不是产品级持续 frame loop，也不能加载 Web/Cocos/native target core bootstrap、普通插件、Runtime QPK executable dependency、文件系统/网络资源或 native payload。
...
- UI surface 的绘制和命中测试。
- 当前文本路径对普通 ASCII、常见 UI 标点、unsupported non-whitespace glyph 的逐字符 solid fallback block、normal / italic font style、已解析 native `text-align` / `white-space` / `text-overflow` 子集，且 `text-decoration` 为 `none` / `underline` / `line-through` 的 Text / RichText / Button label 生成确定性的内建 bitmap TextAtlas fallback；real WGPU 路径会采样内建 atlas texture 来绘制 atlas glyph、unsupported-glyph solid mask 和 solid decoration mask。`pre` / `pre-wrap` 的连续空白会保留，bitmap 路径使用 bitmap glyph 宽度做换行，`clip` / `ellipsis` 会做确定性裁剪和省略号 marker。它还不是真实 CJK 字形渲染、font shaping、bidi、字体 fallback 或字体资源 rasterization 后端，更复杂 typography 仍可能需要 placeholder geometry 或未来文本后端。
...
- JSON facade 必须在 serde 默认值生效前要求顶层 `view` 显式存在。native app / QuickJS bridge 不能把 `{}`、只有 `layout` 或只有 `container` 的输入静默解释为空 projection frame，否则会掩盖 engine projection 丢失并错误推进 renderer revision。
...
- JSON facade 还要要求已解析图片资源显式携带 `assetType`，包括 dialogue avatar、UI node `image` 和 UI style `backgroundImage`。Rust DTO builder 可以继续为了内部测试默认 `images`，但 native app / QuickJS bridge JSON 不能把缺失 `assetType` 的资源静默变成 `images:*` resource id，否则会掩盖 compiler / tooling 没有输出完整 resolved projection 的问题。
- JSON facade 还要要求已解析 dialogue projection 显式携带 `visible` 和 `mode`。Rust DTO builder 可以继续为了内部测试默认 visible / `say`，但 native app / QuickJS bridge JSON 不能把缺失的 dialogue 可见性静默解释成显示对话，也不能把缺失 `mode` 的 dialogue 静默当作 `say`，否则会掩盖 engine / compiler 没有输出完整 projection，并可能改变隐藏/显示或 narration/say 的渲染语义。
- JSON facade 还要要求已解析 choices projection 显式携带 `view.choices.visible`、`view.choices.choices` 和每个 choice 的 `enabled`。Rust DTO builder 可以继续为了内部测试默认 visible / enabled，但 native app / QuickJS bridge JSON 不能把缺失的选择集可见性、选项数组或可点击状态静默解释成可见且可点击。
- JSON facade 还要要求已解析 character projection 显式携带每个角色的 `visible`。Rust DTO builder 可以继续为了内部测试默认 visible，但 native app / QuickJS bridge JSON 不能把缺失的角色可见性静默解释成显示角色。
- JSON facade 还要要求已解析 UI projection 显式携带 `view.ui.visible`。Rust DTO builder 可以继续为了内部测试默认 UI visible，但 native app / QuickJS bridge JSON 不能把缺失的 UI 可见性静默解释成显示 overlays。
- JSON facade 还要要求每个已解析 `UiSurfaceNodeProjection.visible` 显式存在。`@quajs/native-ui-compiler` 在没有 QUI `show` 或 QSS `display` / `visibility` 覆盖时必须输出 `visible: true`；native app / QuickJS bridge JSON 不能让 Rust serde 默认把 malformed QUI/QSS surface node 静默解释成可见 UI。
- JSON facade 还要拒绝已解析 UI text 中的不安全 payload，例如超过 native text payload 上限或包含不支持的控制字符。`UiSurfaceNodeProjection.text` 会进入 button / text draw params 和后续 glyph layout，因此必须在 frame preparation 前失败；这不是 QUI content model、文本插值、本地化或 rich text authoring 语义校验，那些仍然属于 TS compiler / LSP。
- JSON facade 还要拒绝已解析 dialogue / rich-text 中的不安全文本 payload，包括 plain text、speaker text、span text 以及 `rich_text_to_plain_text` 合并后的总字节数。它们会进入 `TextDrawParams` 和后续 glyph layout，因此必须在 frame preparation 前失败；dialogue authoring policy、本地化、插值和 rich-text syntax 仍然属于 engine/compiler/tooling。
- JSON facade 还要拒绝已解析 UI intent metadata 中的不安全 key 和超限 payload。metadata key 复用 native dispatch identifier 规则，metadata payload 需要限制 entry 数、字符串大小、总估算字节、array/object fanout 和嵌套深度，因为它会进入 `NativeRendererIntent.payloadJson` 并交给 native host / `@quajs/engine-native` 桥接；这只是 resolved payload guard，不能变成 Rust 侧业务 action parser。
- JSON facade 还要要求已解析 UI intent 显式携带 `event`。Rust DTO builder 可以继续为了内部测试默认 `ui/intent`，但 native app / QuickJS bridge JSON 不能把缺失 `event` 的 intent 静默变成 `ui/intent`，否则会掩盖 compiler / tooling 没有输出 renderer event envelope 的错误。
...
- JSON facade 还要要求 native audio track 显式携带 `assetType`、`loadMode` 和 `playbackState`。Rust DTO builder 可以继续为了内部测试默认它们，但 native app / QuickJS bridge JSON 不能把缺失 `assetType` 的 `voice` / `sfx` / `ambient` track 静默变成 `bgm` resource，不能把缺失加载策略静默当作 `buffered`，也不能把缺失播放意图静默当作 `playing`。
- JSON facade 还要拒绝 Web/audio projection alias 混入 native bridge JSON，例如 `assetKey`、`state`、`loop`、`playing`。native 已解析输入必须使用 `assetName`、`playbackState`、`looped` 等 native projection 字段；不能让 serde 忽略未知字段后靠默认值继续渲染，也不能在 Rust 侧把 Web 字段重新解释成 native 语义。
...
- texture upload sync: pending / resident / orphaned resident texture ids from backend snapshots
- host texture sync: pending request -> mounted bundle candidate -> package-relative asset bytes -> WGPU upload sink, with owner / required package provenance preserved for resident texture memory and unload checks; package-scoped requests must fail as missing when no mounted bundle matches owner/required package candidates, rather than falling back to unscoped host asset reads
- orphaned resident texture cleanup: host texture sync must pass `NativeRendererFrameResult.texture_upload_sync.orphaned_resident_resource_ids` to the upload sink release hook; the real WGPU sink releases decoded resident textures through `release_decoded_texture`, invalidating sampler cache entries, and release failures must be reported as texture sync errors rather than silently ignored
- host cleanup texture release: package release / renderer clear paths should use `release_package_resources_with_host_texture_cleanup` / `clear_renderer_with_host_texture_cleanup` or their audio-teardown variants in `quajs_native_app::texture_sync`; these helpers call the renderer release/clear API first, then pass returned `NativeRendererHostCleanupRecord` entries through `sync_texture_releases_from_host_cleanup`. The sync step may release only `Texture` and `DecodedImage` records through the upload sink release hook, report missing already-cleared texture handles as non-fatal, and ignore declarative/UI/resource records such as `UiAst`, `QssStyle`, and `TokenTable` instead of treating them as GPU textures. Package unload blockers must continue to come from renderer state; blocked package release must not clean resident texture handles.
- mounted bundle lifecycle texture cleanup: `NativeTextureBundleMountRegistry` stores the last host-observed runtime package bundle set from `NativeHostApi::list_mounted_bundles`, and `sync_mounted_texture_bundle_lifecycle_from_host` compares it with the current host set. First sync establishes a baseline only; later host unmounts call renderer package release plus host texture cleanup for removed packages. Packages blocked by active renderer unload guards remain tracked so later lifecycle syncs retry instead of dropping resident texture handles. Product frame loops should prefer `render_frame_with_host_texture_lifecycle_sync`, which runs `render_frame_with_host_texture_sync` first and only then runs mounted-bundle lifecycle cleanup, so the current projection can release stale frame texture references before package-unmount cleanup evaluates renderer unload blockers. This is app/host transient resource management, not Rust ownership of runtime package authority.
- texture-synced frame helpers: `render_frame_with_host_texture_sync` must prepare the frame, release any texture / decoded-image records from `NativeRendererFrameUpdate.host_cleanup` through the upload sink release hook before submitting the current frame, submit once to discover pending texture resources, resolve host uploads, release remaining orphaned resident textures, and resubmit the same prepared frame only after successful uploads. When only cleanup/release happened, the helper refreshes or preserves a post-cleanup texture sync snapshot without resubmitting; the JSON helper must parse and validate `NativeRendererJsonFrameInput` before any host read, so unsafe resolved asset refs fail without touching QuaAssets/native storage.
- real-wgpu presentation staging: the real runtime can copy a completed offscreen frame target into an external `COPY_DST` texture, consume a `wgpu::SurfaceTexture` through `present_frame_to_surface_texture`, or acquire from an externally owned `wgpu::Surface` through `present_frame_to_surface`, which prepares the swapchain-present bridge.
- real-wgpu surface bootstrap/config planning: `create_real_wgpu_surface_target` can select a surface-compatible adapter, request device/queue, configure an externally owned surface, and return a `RealWgpuNativeRenderRuntimeTarget`; `configure_wgpu_surface_for_native_renderer` selects format/present/alpha modes and requires `RENDER_ATTACHMENT | COPY_DST` on surface textures; `resize_target_from_surface_config` rebuilds frame uniforms and the offscreen frame target after external surface resize while preserving reusable buffers/textures and clearing pipeline caches only when the color format changes.
- native-window smoke: `quajs_native_app` has a `native-window` feature that wires `image-decode`, `winit` window creation, real surface bootstrap, in-memory host texture bytes, real WGPU texture upload/resubmit, basic resize/scale-factor surface reconfiguration, Lost/Outdated present recovery through surface reconfigure + backend target resize, one-shot real wgpu backend rendering, winit cursor/mouse to native pointer-event translation, deterministic resolved-UI pointer probing through `NativeHostApi`, and present attempt reporting behind `QUA_NATIVE_RENDERER_WINDOW_SMOKE=1`. It is intentionally a smoke/debug path; continuous app frame scheduling, full device/surface lifecycle recovery, and product input/focus/IME loops remain future product loop work.
