...
- JSON facade 还要要求 native audio track 显式携带 `assetType` 和 `playbackState`。Rust DTO builder 可以继续为了内部测试默认它们，但 native app / QuickJS bridge JSON 不能把缺失 `assetType` 的 `voice` / `sfx` / `ambient` track 静默变成 `bgm` resource，也不能把缺失播放意图静默当作 `playing`。
