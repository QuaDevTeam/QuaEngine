...
- Rust JSON facade required audio field validation：resolved native JSON 中每个 audio track 必须显式提供 `assetType` 和 `playbackState`。测试要覆盖缺失 `assetType` 的 `voice` track 不会被 serde 默认成 `bgm` resource、缺失 `playbackState` 不会默认成 `playing`，并确认失败后 renderer revision / frame 不前进。直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
