...
- Rust JSON facade required dialogue mode validation：resolved native JSON 中的 `view.dialogue.mode` 必须显式提供，不能让 serde 默认成 `say`。测试要覆盖缺失 `mode` 的 dialogue projection 返回 validation error，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
