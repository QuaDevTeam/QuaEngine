...
- Rust JSON facade resolved UI content-model validation：resolved native JSON 中 `Text` / `RichText` / `Image` / `Divider` / `Spacer` 等 leaf/no-content node 不能携带非空 `children`。Compiler/LSP 仍负责 QUI authoring content model 诊断；Rust 只作为 native app / QuickJS bridge 的 resolved projection 防线，测试要覆盖 leaf node 带 child component 时返回 `.children` validation error，并确认 renderer revision / frame 不前进。
