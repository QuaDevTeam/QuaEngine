...
- Rust JSON facade top-level required projection validation：`NativeRendererJsonFrameInput.view` 必须在 native app / QuickJS bridge JSON 中显式存在。测试要覆盖缺失 `view` 的 `{ layout/container only }` 输入返回 validation error，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 仍可使用默认空 view 做内部单测辅助。
...
- Rust JSON facade resolved UI required-field validation：`UiSurfaceNodeProjection.kind` 必须由 TS compiler / tooling 在 resolved projection JSON 中显式写出；JSON bridge 要在 serde 默认值生效前拒绝缺失或 `null` 的 root / children node `kind`，并确认失败后 renderer revision / frame 不前进。直接构造 Rust DTO 的内部 builder 可以继续使用默认值，但 native app / QuickJS bridge 不能把 malformed QUI/QSS projection 静默解释成 `Box`。
- Rust JSON facade resolved UI supported-kind validation：`UiSurfaceNodeProjection.kind` 必须是 foundational native-wgpu node kind。测试要覆盖 `Dialog` / `Drawer` 等 authoring-time composite 名称直接进入 resolved JSON 时返回 `.kind` validation error，并确认 renderer revision / frame 不前进；composite 展开仍属于 TS compiler/tooling，不属于 Rust renderer。
- Rust JSON facade resolved image resource required-field validation：dialogue avatar、UI node `image`、UI style `backgroundImage` 必须在 resolved native JSON 中显式提供 `assetType`，不能让 serde 默认成 `images`。测试要覆盖 `view.dialogue.avatar.assetType`、`view.ui.overlays[0].surface.root.image.assetType` 和 `view.ui.overlays[0].surface.root.style.backgroundImage.assetType` 缺失时返回 validation error，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade resolved UI geometry validation：`UiSurfaceNodeProjection.bounds` 如果出现必须是对象，`bounds.x/y/width/height` 和 `scrollOffsetX` / `scrollOffsetY` 必须在 frame preparation 前校验为有限 logical stage 数值；`width` / `height` 必须非负并在 native renderer 上限内；scroll offsets 也必须在上限内。该测试要覆盖 malformed bounds shape、negative dimension、non-finite value、oversized coordinate / dimension / scroll offset，并确认失败后 renderer revision / frame 不前进。该校验只能消费 resolved projection JSON，不能把 QUI/QSS parser、selector matching、cascade 或 renderer-owned layout state 放进 Rust。
...
- Rust JSON facade required dialogue field validation：resolved native JSON 中的 `view.dialogue.visible` 和 `view.dialogue.mode` 必须显式提供，不能让 serde 默认成显示对话或 `say`。测试要覆盖缺失 `visible` 和缺失 `mode` 的 dialogue projection 返回 validation error，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade required choices visibility validation：resolved native JSON 中的 `view.choices.visible`、`view.choices.choices` 和每个 choice 的 `enabled` 必须显式提供，不能让 serde 默认成可见、空数组或可点击。测试要覆盖缺失 choice set `visible`、缺失 choice set `choices`、缺失 choice `enabled`，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade required character visibility validation：resolved native JSON 中每个 `view.characters[*].visible` 必须显式提供，不能让 serde 默认成显示角色。测试要覆盖缺失 character `visible`，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade required UI visibility validation：resolved native JSON 中的 `view.ui.visible` 必须显式提供，不能让 serde 默认成显示 UI overlays。测试要覆盖缺失 UI `visible`，并确认 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade required UI surface node visibility validation：resolved native JSON 中每个 `UiSurfaceNodeProjection.visible` 必须显式提供，不能让 serde 默认成显示 UI node。`@quajs/native-ui-compiler` 测试要证明默认节点、`rootId` 包装 `Fragment`、QSS `display: none` / `visibility: hidden` 和 QUI `show` override 都输出显式 visible；Rust 测试要覆盖缺失 surface node `visible` 返回 validation error，并确认 renderer revision / frame 不前进。
- Rust JSON facade resolved UI content-model validation：resolved native JSON 中任一 UI surface node 的 `children` 字段如果出现就必须是数组，`Text` / `RichText` / `Image` / `Divider` / `Spacer` 等 leaf/no-content node 不能携带非空 `children`。Compiler/LSP 仍负责 QUI authoring content model 诊断；Rust 只作为 native app / QuickJS bridge 的 resolved projection 防线，测试要覆盖非数组 `children` 和 leaf node 带 child component 时返回 `.children` validation error，并确认 renderer revision / frame 不前进。
...
- Rust JSON facade required UI intent event validation：resolved native JSON 中每个 overlay / surface node `intent` 必须显式提供 `event`，不能让 serde 默认成 `ui/intent`。测试要覆盖 overlay intent 和 surface node intent 缺失 `event` 的输入，并确认失败后 renderer revision / frame 不前进；直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
...
- Rust JSON facade required audio field validation：resolved native JSON 中每个 audio track 必须显式提供 `assetType`、`loadMode` 和 `playbackState`。测试要覆盖缺失 `assetType` 的 `voice` track 不会被 serde 默认成 `bgm` resource、缺失 `loadMode` 不会默认成 `buffered`、缺失 `playbackState` 不会默认成 `playing`，并确认失败后 renderer revision / frame 不前进。直接 Rust DTO builder 的默认值不作为 native app / QuickJS bridge 输入契约。
- Rust JSON facade unsupported audio field validation：resolved native JSON 中的 audio track 必须拒绝 Web/audio alias 字段，例如 `assetKey`、`state`、`loop`、`playing`，并提示使用 native 字段 `assetName`、`playbackState`、`looped`。测试要确认这些字段在 serde 默认值生效前失败，renderer revision / frame 不前进，避免 Web/Cocos/native projection 字段串线。
