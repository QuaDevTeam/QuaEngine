...
- Rust JSON facade resolved background / video number validation：`BackgroundProjection` 和 `BackgroundLayerProjection` 的 `x/y`、可选 `width/height`、`scale`、`opacity`，以及 `BackgroundVideoProjection.opacity` 必须在 frame preparation 前校验为安全数值；坐标必须有限并在上限内，尺寸必须有限、非负且在上限内，scale 必须有限、大于 0 且在上限内，opacity 只能是有限 `0..=1` normalized value。测试要覆盖 negative background dimension、invalid layer scale、invalid video opacity，并确认失败后 renderer revision / frame 不前进。该路径只保护 background layout 和 video poster/fallback projection，不能声明真实 video decode capability，也不能把 Rust 变成 QSS parser 或 clamp malformed projection。
