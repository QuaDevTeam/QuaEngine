...
`Image` 是 package-backed leaf。`src` / `image` 缺失、非字面量或不安全时应由 compiler / LSP 诊断并避免生成有效 `image` projection；Rust/wgpu 只消费 resolved asset type/name，遇到空 resolved image asset 时跳过该 leaf 的空 draw command，而不是替作者猜测 fallback 资源。
...
- `font-weight`
- `gap` / `row-gap` / `column-gap`（native 子集：一到两个非负 logical px / unitless number；作为 compiler-only layout metadata，被 `compileNativeUiSurfaceProjection` 展开到 `Row` / `Column` / `Grid` 直系子节点的 resolved `bounds`；Rust 不接收也不解析 gap 字段，这不是完整 CSS flex/grid layout）
- `justify-content`（native 子集：`flex-start` / `center` / `flex-end` / `space-between` / `space-around` / `space-evenly`；作为 compiler-only layout metadata，只影响 `Row` / `Column` 直系子节点主轴分布，展开到 resolved child `bounds`；Rust 不接收也不解析 justify 字段，这不是完整 CSS flexbox）
- `align-items`（native 子集：`flex-start` / `center` / `flex-end`；作为 compiler-only layout metadata，只影响 `Row` / `Column` 直系子节点交叉轴对齐，展开到 resolved child `bounds`；Rust 不接收也不解析 align 字段，也不承诺 `stretch` / `baseline`）
- `margin` / `margin-top` / `margin-right` / `margin-bottom` / `margin-left`（native 子集：一到四个非负 logical px / unitless number；作为 compiler-only layout metadata，只由 `Row` / `Column` / `Grid` 直系子节点排布消费，展开到 resolved child `bounds`；Rust 不接收也不解析 margin 字段，这不是完整 CSS box model / flow layout）
- `position`（native 子集：`relative` / `absolute`；作为 compiler-only layout metadata，`absolute` 仅让 `Row` / `Column` / `Grid` 直系子节点退出结构 layout flow，并按 already-resolved `bounds` 相对结构父节点投影；Rust 不接收也不解析 position 字段，这不是完整 CSS positioning）
...
- `pointer-events`（native 子集：`auto` / `none`；作为 node-local compiler-only 交互元数据，`none` 会在 projection 编译时移除该节点自己的 resolved intent，但不隐藏节点、不禁用视觉样式、也不阻止子节点继续生成自己的 intent；Rust 仍只消费最终 intent 投影，不解析 QSS 文本）
