...
- Packaging to Web, Cocos, or native must start from the selected target resolver. Do not construct a combined Web/Cocos/native core plugin set and filter it later; inactive target core plugins must never be imported, registered, bundled, or preserved as executable QPK dependencies.
- Packaging into Cocos, Web, and native projects must keep core plugins on separate rails end to end. Cocos packaging may only materialize the Cocos core plugin family, Web packaging may only materialize the Web core plugin family, and native packaging may only materialize the native core plugin family. The same rule applies to starter templates, generated bootstrap files, debug shells, smoke runners, installers, update manifests, and Runtime QPK executable dependency lists; none of them may carry the other two targets' core plugins and rely on later filtering.
- Cocos, Web, and native project generators must be manifest consumers, not secondary core-plugin assemblers. They may share platform-neutral schema, graph normalization, and manifest validation helpers, but they must never share a template, preset, generated resolver, debug shell, installer, updater, or smoke runner that materializes more than one target core family before filtering.
- Generated Web, Cocos, and native project templates are manifest consumers, not core-plugin injection points. Re-declaring the active target core from a template, startup shell, debug shell, installer, updater, smoke runner, or native app bootstrap is a packaging failure even when no foreign target core is present.
- The canonical target-core isolation gate is `.agents/native-renderer/target-core-isolation.md`; packaging, project generator, starter, debug/release shell, installer, updater, smoke runner, and Runtime QPK resolver plans must keep that document and the matching `tooling-testing.md` fixtures in sync.
...
- 2026-07-05 dependency-pin 复核结论：`packages/native/crates/quajs_wgpu_renderer` 当前 pin `wgpu = 29.0.3`；native renderer 计划、benchmark 和真实设备 smoke 以该版本为当前验收基线，后续升级 wgpu 时需要单独跑 real-wgpu / window / texture upload / buffer benchmark 回归。
- wgpu provides a safe Rust graphics API over Vulkan, Metal, D3D12, OpenGL, WebGPU, and WebGL. Current native work should validate against the pinned `wgpu = 29.0.3` crate version before considering a wgpu upgrade. Sources: https://github.com/gfx-rs/wgpu, https://docs.rs/wgpu/
...
- `rquickjs` exposes QuickJS to Rust with runtime/context, async integration, and custom resolver/loader support. Do not enable `dyn-load` for QuaEngine native runtime packages. Source: https://docs.rs/rquickjs
...
- Taffy implements CSS Block, Flexbox, and Grid layout algorithms in Rust. It is a candidate for native UI layout internals only after QUI/QSS compiler semantics are fixed; it must not turn Rust into a QSS parser. Source: https://github.com/DioxusLabs/taffy
...
- VS Code's language-server extension model supports a separate language client and server over LSP; the current LSP specification is 3.18. Sources: https://code.visualstudio.com/api/language-extensions/language-server-extension-guide, https://microsoft.github.io/language-server-protocol/specifications/lsp/3.18/specification/
...
Core target plugins are reserved bootstrap infrastructure, not normal game plugins. Project authors and third-party packages must not place `web-core`, `cocos-core`, or `native-core` packages into the ordinary plugin list, Runtime QPK executable dependency list, or shared preset exports. Quack should materialize a separate immutable `TargetCoreSelection` before it resolves normal plugins:

```ts
interface TargetCoreSelection {
  target: 'web' | 'cocos' | 'native'
  targetCoreResolver: 'web-core-resolver' | 'cocos-core-resolver' | 'native-core-resolver'
  ...
  forbiddenCoreFamilies: readonly ('web-core' | 'cocos-core' | 'native-core')[]
  forbiddenCoreAdapterRoots: readonly string[]
}
```

Only the active target resolver may create this object. Its `targetCoreResolver` value must be written into the emitted `target-bundle-manifest.json` and must match the target: `web` uses `web-core-resolver`, `cocos` uses `cocos-core-resolver`, and `native` uses `native-core-resolver`. Shared plugin resolution consumes the selection as read-only input and must fail if any plugin, subentry, QPK executable dependency, or generated bootstrap record attempts to add another target core family. This keeps Web, Cocos, and native packaging paths from drifting while still allowing platform-neutral game plugins to be reused.
...
Concrete project packaging rule: when generating a Cocos, Web, or native project, the project template, generated startup shell, debug/release shell, installer, updater, smoke runner, and Runtime QPK resolver are manifest consumers only. They must not carry a core plugin list, re-declare even the active target core, or temporarily materialize another target's core plugin for later filtering. A Cocos project consumes only the Cocos manifest, a Web project consumes only the Web manifest, and a native project consumes only the native manifest.
...
3. **Post-bundle graph gate**: every debug and release artifact emits `target-bundle-manifest.json` after bundling/tree-shaking. The manifest records `target`, `targetCoreResolver`, `profile`, `platform`, `app.bundleId`, `app.version`, `selectedCoreAdapters`, normalized dependency roots, selected renderer entries, included QPK ids, and for native artifacts a `nativeRenderer` summary with package, version, backend, capability ids, and capability manifest hash. Packaging fails if `validateTargetBundleManifest` from `@quajs/native-contracts` fails on the post-bundle graph; that helper runs `validateExclusiveTargetBootstrap`, target-core resolver checks, target-specific forbidden/missing checks, explicit selected core adapter checks, target-scoped native renderer metadata checks, app/runtime renderer entry target checks, and Runtime QPK executable dependency/renderer entry checks from the same target manifest data.
...
The same five gates must run for `--target web`, `--target cocos`, and `--target native`. Native packaging cannot be the only strict path; Web and Cocos builds must reject native core adapters with the same severity that native builds reject Web/Cocos adapters.
...
The target bundle manifest is the handoff contract between source selection, bundling, release packaging, and runtime startup. It must be generated after tree-shaking for each output, not copied from project source metadata. A release or debug artifact is invalid when any of these fields disagree: `target`, `targetCoreResolver`, `selectedCorePluginFamily`, `selectedCoreAdapters`, normalized runtime dependency roots, selected renderer entries, Runtime QPK executable dependencies, and app/runtime renderer entry target metadata.
...
- The packager must fail before signing/release promotion when `target`, `targetCoreResolver`, `selectedCorePluginFamily`, and `selectedCoreAdapters` disagree. For example, `target: "native"` with `targetCoreResolver: "web-core-resolver"` or a Web renderer entry is invalid even if all native adapters are also present.
...
- `packages/native/contracts/test/target-bundle.test.ts` covers clean Web/Cocos/native target-bundle manifests, required `targetCoreResolver`, resolver/target mismatches, required `selectedCorePluginFamily`, target/family mismatches, cross-family package leakage, cross-target core adapter leakage for all three targets, native artifacts containing Web/Cocos renderer entries, Web/Cocos artifacts retaining `@quajs/native-contracts` in their runtime graph, Runtime QPK executable dependencies on target core adapters, incomplete/extra selected core adapters, and renderer entry target mismatches.
- `@quajs/quack/project` now normalizes `targets.native` metadata and exposes `createQuaProjectNativeArtifactPlans`, which expands native `platforms` × `profiles` into version-isolated artifact directories under `outputDir/profile/version-buildNumber/platform` for target-bundle manifest emission and release/debug output separation.
- `@quajs/quack/project` exposes `emitQuaTargetBundleManifest` for Web, Cocos, and native artifacts. It validates a supplied target-bundle manifest with the active `expectedTarget`, then writes `target-bundle-manifest.json` only after validation succeeds. `emitQuaProjectNativeTargetBundleManifest` is the native artifact-plan wrapper that creates native manifest data and delegates to the shared emitter.
- Next Quack/native-packager work must wire the real post-bundle dependency graph into the shared emitter for Web, Cocos, and native debug/release artifacts. Source-level package metadata checks are not enough; validation must run on the emitted dependency graph after tree-shaking for every target.
...
- `@quajs/native-contracts` native runtime package guard scans scripts, scenes, renderer/engine plugins, store migrations, their runtime module variants, and emitted bundle asset names/paths/relative paths/variant names/variant paths before QuickJS evaluation. It rejects absolute, remote, traversal, target-core dependency, native plugin, native-code declaration, and native payload references at runtime even when Quack build-time validation was skipped.
...
- Rust `quajs_wgpu_renderer` now renders supported ASCII Text / RichText / UI Button labels, including common UI punctuation such as braces, currency, caret, tilde, and backtick, plus per-character solid fallback blocks for unsupported non-whitespace glyphs, through a deterministic built-in bitmap TextAtlas fallback for normal and italic font styles across the resolved native `text-align`, `white-space`, and `text-overflow` subsets. It supports `pre` / `pre-wrap` preserved whitespace, bitmap-width wrapping, deterministic `clip` / `ellipsis` overflow markers, and `text-decoration: none`, `underline`, and `line-through`; the real WGPU path samples the built-in atlas texture for atlas glyphs, unsupported-glyph solid masks, and solid decoration masks. This improves native UI legibility without claiming real CJK glyph rendering, font shaping, bidi, font fallback, or a font asset rasterization backend; richer typography may still need placeholder geometry or a future text backend.
- Rust `quajs_wgpu_renderer` now exposes host-facing texture upload sync data on `NativeRendererFrameResult`. Frame preparation produces package-aware `NativeTextureUploadRequestPlan`, including owner / required package provenance; after backend submission the result also includes pending, resident, and orphaned resident texture ids based on the backend's resident texture snapshot. The real-wgpu runtime can consume uploaded RGBA decoded textures, rebuild texture sampler bind groups, and release decoded textures when they become orphaned, while `quajs_native_app` smoke emits `textureUploadPendingRequestCount`, `textureUploadResidentResourceCount`, and `textureUploadOrphanedResidentResourceCount` for CI/budget checks. `quajs_native_app::texture_sync` now provides the host/app bridge for pending texture uploads, frame host cleanup, orphan cleanup, package release cleanup, renderer clear cleanup, and mounted-bundle lifecycle cleanup: it resolves owner-first package candidates through `NativeHostApi::list_mounted_bundles`, reads package-relative asset bytes through `NativeHostApi::read_asset_bytes`, rejects unsafe asset/package references before host reads, preserves selected/required package provenance in upload metadata, releases `NativeRendererFrameUpdate.host_cleanup` texture / decoded-image records before current frame submission, releases orphaned resident texture ids through the upload sink release hook, and has a feature-gated real WGPU sink that calls `upload_image_texture_bytes_with_metadata` / `release_decoded_texture`. `NativeTextureBundleMountRegistry` records the last host-observed runtime package bundle set and `sync_mounted_texture_bundle_lifecycle_from_host` releases texture resources for packages that disappeared from the host bundle list while retaining blocked packages for later retry. `render_frame_with_host_texture_sync` composes texture sync with `NativeRenderer`: it prepares the frame, applies frame host cleanup release, submits once to discover pending textures, uploads them, releases remaining orphaned resident textures, then resubmits the same prepared frame only when uploads succeeded so the final frame result reports resident texture ids; if only cleanup/release happened, it preserves or refreshes a post-cleanup texture sync snapshot without resubmitting. `render_json_frame_with_host_texture_sync` parses and validates `NativeRendererJsonFrameInput` before any host asset read, then delegates to the same texture sync path; `render_json_frame_with_host_texture_lifecycle_sync` extends that JSON bridge with mounted-bundle lifecycle sync in the product-loop order. The real-wgpu runtime also exposes `create_real_wgpu_surface_target` for externally owned surface bootstrap, `resize_target_from_surface_config` / `resize_target` for rebuilding frame uniforms and the offscreen frame target after external resize, `copy_frame_to_texture` for copying a completed offscreen frame target into an external `COPY_DST` texture, `present_frame_to_surface_texture` for copying into a `wgpu::SurfaceTexture` before calling `present()`, and `present_frame_to_surface` for acquiring from an externally owned `wgpu::Surface` before the same copy/present handoff. A `real-wgpu`-gated surface configuration helper now selects surface format/present/alpha modes, calls `surface.configure`, and requires `RENDER_ATTACHMENT | COPY_DST` on swapchain textures. `quajs_native_app` also has a `native-window` feature-gated one-shot smoke path: `QUA_NATIVE_RENDERER_WINDOW_SMOKE=1` opens a `winit` window, bootstraps a real surface target, uses in-memory package-aware fixture textures through the native host texture lifecycle sync helper, applies frame host cleanup release, decodes/uploads those textures into the real WGPU backend, resubmits after upload, runs mounted-bundle lifecycle baseline/cleanup sync, handles basic resize/scale-factor events by reconfiguring the surface and resizing the backend target, translates winit cursor/mouse input into native pointer events, runs a one-shot deterministic resolved-UI pointer probe that emits `ui/intent` through `NativeHostApi`, recovers Lost/Outdated present acquisition by reconfiguring the current surface size and retrying redraw, renders resolved projection JSON through the real backend, and reports `presented` / `presentStatus` plus `resizeCount`, `surfaceRecoveryCount`, texture upload/resident/resubmit counts, texture lifecycle sync/baseline/tracked/blocked/release/cleanup-error counts, pointer event/intent/probe counts, and last resize dimensions in `Qua native window smoke json`; upload/read/release sync failures and frame host cleanup release failures are counted as texture upload errors. `presented=false` with `OccludedAfterRetry` means OS swapchain present was not verified even though window/surface/offscreen render ran. The remaining gap is a product-grade continuous native window/frame loop, full device/surface lifecycle recovery, and product input/focus/IME loops; the Null-backend smoke path still reports readiness only and does not upload textures.
- Product-grade native frame loops that need both frame texture sync and host mounted-bundle cleanup should use `render_frame_with_host_texture_lifecycle_sync`. This wrapper intentionally runs `render_frame_with_host_texture_sync` before `sync_mounted_texture_bundle_lifecycle_from_host`, allowing the current projection to release stale frame texture references before runtime-package unmount cleanup checks renderer unload blockers; tests must keep this ordering fixed so package cleanup is neither prematurely blocked nor double-released.
- Native-window input smoke now treats winit focus and default keyboard bindings as standard native renderer intents through the in-memory `NativeHostApi`, emitting `window/focus`, `window/blur`, and `user/input_command` without adding a second event bus or persisting raw typed text. The machine-readable smoke report should expose `focusIntentEmitCount` and `keyboardIntentEmitCount` alongside focus, keyboard, pointer, texture, resize, and present metrics so CI can verify product-loop input wiring.
- `quajs_native_app::texture_sync` also exposes package release / renderer clear integration helpers. `release_package_resources_with_host_texture_cleanup` / `clear_renderer_with_host_texture_cleanup` and their audio-teardown variants call renderer release/clear first, then consume returned `NativeRendererHostCleanupRecord` lists through `sync_texture_releases_from_host_cleanup`. The cleanup sync releases only `Texture` / `DecodedImage` handles through the same upload sink release hook, treats already-missing handles as non-fatal missing cleanup entries, reports sink release failures, ignores declarative/non-texture ledger records such as `UiAst`, `QssStyle`, and `TokenTable`, and preserves renderer unload blockers so blocked packages do not lose resident texture handles.
...
- Rust `quajs_native_app` now has a small startup gate for native `target-bundle-manifest.json` contract data. It can load an emitted manifest from `QUA_NATIVE_TARGET_BUNDLE_MANIFEST`, validates the native target, `native-core-resolver`, `native-core` family, required native core adapters, app `bundleId`/version/build/profile/platform identity, native renderer package/version/backend/capability metadata, foreign Web/Cocos core roots, renderer entry target metadata, and Runtime QPK executable/renderer entries before building `NativeHostInfo`; tests prove foreign Web/Cocos package roots, resolver mismatches, app identity mismatches, missing native renderer metadata, and invalid manifest JSON fail before host-info construction. After host info is built from signed Rust app metadata, startup also compares the emitted renderer package/version/backend/capability ids and capability manifest hash against actual host info before QuickJS/engine startup. Native shell/bootstrap work still must pass the already validated emitted manifest into `NativeHostPlugin` before engine initialization, and `@quajs/engine-native` repeats the emitted renderer package/version/backend/capability ids/hash comparison before keeping the plugin initialized. `quajs_native_runtime::capability_manifest_hash` computes the host-side `sha256:` hash from the serialized renderer capability manifest so release metadata is not a loose fixture string.
- `@quajs/native-ui-compiler` now supports compiler-only expansion of project/package `.qui` composites through explicit `compileNativeUiSurfaceProjection({ components })` document maps supplied by tooling or packagers. Composite templates can use `props.*` bindings and named slot placeholders, invocation slot content is inserted before projection, loop/composite keys scope expanded child ids, and resolved projections still contain only foundational native-wgpu node kinds. The compiler does not load component files, QPKs, URLs, or native host APIs by itself.
...
- Emit a target bundle manifest for every debug and release artifact with `target`, `targetCoreResolver`, `profile`, `app.version`, `app.bundleId`, selected core adapters, renderer package/version, native capability hash when applicable, and included QPK ids. Release promotion must compare this manifest against `validateExclusiveTargetBootstrap` and `validateTargetBootstrap`.
- Add negative package fixtures where each target is polluted by the other two target core families. For example, Web + native store, Web + Cocos renderer, Cocos + Web renderer, Cocos + native engine, native + Web renderer, and native + Cocos host must all fail from the same validator surface before debug output or release signing is allowed.
...
- `background-position`.
- `object-fit` / `object-position` for package-backed QUI `Image` leaves.
...
- `native.package_release.summary.smoke` exercises package-scoped resource release summaries, blocked unload pressure, and declarative QUI/QSS release/block counts plus byte estimates for repeated runtime package unload scenarios.
...
- Include native renderer package version, backend, capability ids, and capability manifest hash in release artifacts, update channel metadata, and native `target-bundle-manifest.json`.
- Add target isolation checks so Web/Cocos/Native release bundles cannot include another target's core adapter plugins.
- Emit `target-bundle-manifest.json` for Web, Cocos, and native debug/release artifacts with the matching `targetCoreResolver` identity and validate it after bundling/tree-shaking with `validateTargetBundleManifest(manifest, { expectedTarget })`.
...
- Debug and release artifact manifest tests verify `target-bundle-manifest.json` is emitted after bundling/tree-shaking and fails validation when `targetCoreResolver` is missing or belongs to another target, when another target's core plugin appears only through a subentry import, when `selectedCoreAdapters` is incomplete or contains another target's adapter, and when app/runtime renderer entry target metadata does not match the artifact target.
...
## Documents Written To `.agents`

本方案已经拆成可执行文档：

- `.agents/native-renderer/README.md`：native renderer 总览、核心边界和实施顺序。
- `.agents/native-renderer/architecture.md`：packages/native 结构、QuickJS / wgpu / host bridge、动态包、assets/store/native 版本和内存/media。
- `.agents/native-renderer/qui-qss.md`：QUI 语法、条件/循环渲染、QSS CSS 子集、base component、composite 扩展策略。
- `.agents/native-renderer/tooling-testing.md`：独立 native LSP、VSCode plugin、QSS 自动化验收、runtime/packaging tests、benchmark。
- `.agents/native-renderer/target-core-isolation.md`：Web / Cocos / Native 核心插件互斥、项目生成、Runtime QPK、debug/release/installer/updater 隔离。
- `.agents/native-renderer/release-packaging.md`：macOS / Windows 优先、Linux 其次的 app identity、icon、bundleId、版本、加固、签名、debug/release/release-by-version 产物隔离。
- `.agents/native-renderer/development-guidelines.md`：未来 engine 能力迭代规范、Rust native bridge plugin、assets/store/native runtime version、第三方 plugin native renderer 兼容声明。
