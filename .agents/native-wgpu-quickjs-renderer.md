...
- Rust `quajs_wgpu_renderer` now has an audio projection/resource tracking skeleton. It maps engine-owned audio tracks to package-aware `AudioBuffer`, `AudioStream`, and `AudioHandle` ledger records, reports audio sync summaries during `prepare_frame`, preserves backend-provided memory when projection estimates are absent, emits host cleanup records when tracks disappear, and blocks package unload while active audio projections still reference package-owned resources. This is not real audio playback yet and must not be advertised as `native-wgpu.audio@1` until a backend exists.
