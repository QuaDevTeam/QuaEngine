...
- `@quajs/quack/project` exposes `emitQuaProjectNativeTargetBundleManifest` for native artifacts. It creates a native manifest from the artifact plan and post-bundle dependency/renderer/runtime metadata, validates it through `assertTargetBundleManifest(..., { expectedTarget: 'native' })`, then writes `target-bundle-manifest.json` only after validation succeeds.
- Next Quack/native-packager work must wire the real post-bundle dependency graph into this native emitter and add equivalent Web/Cocos manifest emission gates. Source-level package metadata checks are not enough; validation must run on the emitted dependency graph after tree-shaking for every target.
