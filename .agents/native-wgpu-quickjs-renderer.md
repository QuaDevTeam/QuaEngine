...
- Rust `quajs_wgpu_renderer` now renders simple supported ASCII Text / RichText / UI Button labels, including common UI punctuation such as braces, currency, caret, tilde, and backtick, through a deterministic built-in bitmap TextAtlas fallback for normal and italic font styles across the resolved native `text-align`, `white-space`, and `text-overflow` subsets. It supports `pre` / `pre-wrap` preserved whitespace, deterministic `clip` / `ellipsis` overflow markers, and `text-decoration: none`, `underline`, and `line-through`; the real WGPU path samples the built-in atlas texture for fallback glyphs and solid decoration masks. Unsupported glyphs and richer typography fall back to the existing placeholder geometry, so this improves native UI legibility without claiming real font shaping, bidi, font fallback, or a font asset rasterization backend.
