...
- `@quajs/quack/project` now normalizes `targets.native` metadata and exposes `createQuaProjectNativeArtifactPlans`, which expands native `platforms` × `profiles` into version-isolated artifact directories under `outputDir/profile/version-buildNumber/platform` for later target-bundle manifest emission and release/debug output separation.
