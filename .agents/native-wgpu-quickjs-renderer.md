...
- Rust `quajs_native_app` now has a small startup gate for native `target-bundle-manifest.json` contract data. It can load an emitted manifest from `QUA_NATIVE_TARGET_BUNDLE_MANIFEST`, validates the native target, `native-core` family, required native core adapters, foreign Web/Cocos core roots, renderer entry target metadata, and Runtime QPK executable/renderer entries before building `NativeHostInfo`; tests prove foreign Web/Cocos package roots fail before host-info construction and invalid manifest JSON is reported at startup. Native shell/bootstrap work still must pass the already validated emitted manifest into `NativeHostPlugin` before engine initialization.
