...
- Rust `quajs_native_app` now has a small startup gate for native `target-bundle-manifest.json` contract data. It validates the native target, `native-core` family, required native core adapters, foreign Web/Cocos core roots, renderer entry target metadata, and Runtime QPK executable/renderer entries before building `NativeHostInfo`; tests prove foreign Web/Cocos package roots fail before host-info construction. Native shell/bootstrap work still must read the real emitted manifest from packaged artifacts and pass it into `NativeHostPlugin` before engine initialization.
