...
- Rust `quajs_native_app` now has a small startup gate for native `target-bundle-manifest.json` contract data. It can load an emitted manifest from `QUA_NATIVE_TARGET_BUNDLE_MANIFEST`, validates the native target, `native-core` family, required native core adapters, app `bundleId`/version/build/profile/platform identity, native renderer package/version/backend/capability metadata, foreign Web/Cocos core roots, renderer entry target metadata, and Runtime QPK executable/renderer entries before building `NativeHostInfo`; tests prove foreign Web/Cocos package roots, app identity mismatches, missing native renderer metadata, and invalid manifest JSON fail before host-info construction. Native shell/bootstrap work still must pass the already validated emitted manifest into `NativeHostPlugin` before engine initialization, and `@quajs/engine-native` now compares the emitted renderer package/version/backend/capability ids and capability manifest hash against actual host info before keeping the plugin initialized. `quajs_native_runtime::capability_manifest_hash` computes the host-side `sha256:` hash from the serialized renderer capability manifest so release metadata is not a loose fixture string.
