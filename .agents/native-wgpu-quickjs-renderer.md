...
- Rust `quajs_native_runtime` owns a package-aware QuickJS module namespace registry with per-namespace package/asset/kind/byte accounting plus namespace, package, and full clear release paths. This is the current placeholder ledger for future real QuickJS module handles and post-unload memory checks.
