...
- Rust `quajs_wgpu_renderer` now has an audio projection/resource tracking and backend-command skeleton. It maps engine-owned audio tracks to package-aware `AudioBuffer`, `AudioStream`, and `AudioHandle` ledger records, reports audio sync summaries during `prepare_frame`, exposes audio asset request plans for buffer/stream resources, preserves backend-provided memory when projection estimates are absent, emits host cleanup records when tracks disappear, blocks package unload while active audio projections still reference package-owned resources, derives `LoadAsset`/`StartTrack`/`UpdateTrack`/`StopTrack`/`ReleaseHandle` command plans for a future `NativeAudioBackend`, lets `NativeRenderer` explicitly apply those plans to an injected backend, and reports audio-specific resource counts/memory/package metrics from the resource ledger. `NullNativeAudioBackend` records these plans for tests only. This is not real audio playback yet and must not be advertised as `native-wgpu.audio@1` until a decoder/mixer/device backend exists.
