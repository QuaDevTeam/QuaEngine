...
The native-window TTF/OTF backend consumes loaded QPK faces and builds transient high-resolution per-frame glyph atlases with proportional metrics. Explicit projected families are selected first and the first active face is the default for text without a family. Native OpenType shaping uses `rustybuzz` to project glyph ids, ligatures, kerning, script-specific clusters, offsets, and horizontal RTL direction into the existing atlas geometry; the atlas keeps the older character map as a backward-compatible fallback for native backends that do not publish shaping data. WGPU applies layout in physical DPR-scaled stage coordinates and samples uploaded font atlases linearly. Projected weights at 500 and above receive a small DPR-scaled horizontal mask embolden when the selected atlas does not expose a distinct weight texture, so regular-only demo faces do not silently flatten every heading to regular weight. A real projected bold face remains preferred. The built-in `5x7` atlas is fallback-only when no usable registered atlas/layout exists or a requested style is not supported by the high-resolution path.

This high-resolution atlas path is not yet full Web typography parity: mixed-direction bidi paragraph segmentation, variable-font axes, WOFF/WOFF2 decoding, advanced CJK line breaking, and high-resolution italic/text-decoration geometry remain separate work. Register faces that cover every required script; a Latin-only Noto Sans file does not provide complete CJK coverage.
