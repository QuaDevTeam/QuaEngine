...
- Creator transforms convert the host's top-left/+Y-down logical coordinates and anchor pivots to Creator parent-local/+Y-up positions and inverted rotation. Keep logical hit testing independent of native coordinates. Resolve `layout.aspectRatio` as a fixed ratio in the platform-neutral layout helper as well as Web; min/max ratios describe the safe area, not a resizable narrative stage.
- Creator input must normalize numeric key codes and convert UI positions into root-container coordinates exactly once. Subscribe to one touch source. A feature must resolve the frontmost interactive target across all feature metadata keys before dispatching, so overlays occlude underlying choices.
- `CocosNodeHost.onLayoutChange` is optional; Creator and fake hosts implement it. Snapshot subscribers must refresh on parent resize, `setView`, `setAssets`, and asset changes. Projection tasks coalesce asynchronous work and cancel obsolete/teardown work before touching nodes; late resources must be released and shared creation must have one native owner.
- Creator media and storage bridges must report real capabilities. Image/audio/font/video byte decoding requires `resources.createResource`; audio playback requires `audio.createAudioHandle`. The built-in Creator bridge does not render video, filters, masks, custom blend/composition, frame crops, or UI skin state transitions; warn once for unsupported projections. Preserve native handles without JSON cloning. Keep Label/RichText and Sprite on separate transient presentation children, and never mutate shared SpriteFrame slice insets.
- `CocosFileStoreBackend` rejects overlapping/nested transactions explicitly; callers must await the active transaction. Failed rollback snapshot capture must not disable subsequent rollback. `clearGameSlots` also removes orphaned payload/preview records. Cocos cache indexes serialize writes and store asset bytes in their own directory.
- `createCocosStaticOnlyTrustPolicy({ allowStaticRuntimePackages: true })` requires an injected `verifyIntegrity` callback unless integrity checking is explicitly disabled for development. `verifyCocosBundleIntegrity` only compares metadata; matching package-declared hashes are not proof that bytes or signatures were verified.
- Read `docs/design/cocos-integration-audit.md` for the static audit scope and remaining Creator integration validation. Fake-host tests are not Creator, device, visual, audio, or durable-storage validation.
- All Cocos panels must refresh from snapshot subscriptions (including public view/asset setters and safe-area changes). Async settings skins use the cancellable projection task; settings hit tests resolve the frontmost control across features. Font registration identity includes asset revision and package candidates so replacement assets cannot leave stale native fonts.
- Each successful host resource create/load acquires one lease. Creator and fake hosts coalesce pending creation, validate returned IDs/kinds, and release native assets only after all owners release. Native audio handles keep their own resource lease through asynchronous stop/dispose, including replacement and renderer teardown. Solid color effects/fades require Creator `cc.Graphics` (optional module capability); node metadata alone does not draw a rectangle.
- Creator projection nodes and their presentation children inherit the parent native render layer so a Canvas camera using `UI_2D` can see them. In-flight native audio creation retains its asset through cancellation even when renderer teardown finishes before the bridge returns.
