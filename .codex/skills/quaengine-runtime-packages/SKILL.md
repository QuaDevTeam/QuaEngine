...
- Runtime script factories may return `GameStep[]` synchronously or through a Promise. `RuntimeContentManager` must await package script factories so native JavaScriptCore, Web loaders, and other injected module loaders can bridge evaluated script modules without faking synchronous host execution.
