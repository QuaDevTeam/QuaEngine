...
- `@quajs/native-language-server`: standalone `.qui/.qss` LSP adapter, in-memory project index, document-link helpers, and component/class reference queries over `@quajs/native-ui-compiler`. It is an authoring tool only; it must not load Web/Cocos/native target core bootstrap plugins, ordinary game plugin arrays, or runtime QPK executable dependencies.
