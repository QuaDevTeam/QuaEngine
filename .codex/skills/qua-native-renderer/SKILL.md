...
- Runtime startup must repeat the exclusive-target assertion before engine initialization so hand-built shells cannot register zero or multiple target core adapter sets. Native startup should pass the emitted `target-bundle-manifest.json` into `NativeHostPlugin` when available, falling back to `targetBootstrapPackages` only for lightweight tests or hand-built debug shells.
