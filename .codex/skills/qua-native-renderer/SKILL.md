...
- Rust `quajs_native_app` startup must validate native `target-bundle-manifest.json` data before building `NativeHostInfo` or starting QuickJS/engine. It can load the emitted manifest from `QUA_NATIVE_TARGET_BUNDLE_MANIFEST` and should reject unreadable/invalid manifest files, foreign Web/Cocos core roots, non-native renderer entry targets, and Runtime QPK target-core executable/renderer entries before host-info construction.
