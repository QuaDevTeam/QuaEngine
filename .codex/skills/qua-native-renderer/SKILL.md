...
- Rust `quajs_native_app` may use `QUA_NATIVE_RENDERER_SMOKE_FRAME` to read an already-resolved projection JSON fixture and submit it through `NativeRendererJsonFrameInput` plus `NullNativeRenderBackend` as a debug/CI smoke check. It should emit both a human-readable summary and a machine-readable `Qua native renderer smoke json: ...` line with missing-resource, fallback, declarative resource, memory, audio resource, and active audio track metrics. This hook must not load QUI/QSS authoring files, ordinary plugins, Runtime QPK executable dependencies, or any native payloads.
