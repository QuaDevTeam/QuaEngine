...
- Current UI/QSS resource tracking in `quajs_wgpu_renderer` records package-aware `UiAst`, `QssStyle`, and `TokenTable` entries with deterministic labels and conservative CPU-byte estimates for memory ledger, budget, metrics, and package unload summaries. Asset planning and frame metrics identify these as declarative asset requests so dynamic QUI/QSS/token package load pressure can be benchmarked separately from image/audio/media resources. These are internal resource-ledger estimates only; real host/backend-reported memory must be preserved when available.
