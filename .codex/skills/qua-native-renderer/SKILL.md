...
- Current audio work in `quajs_wgpu_renderer` is renderer-local resource tracking and backend command planning only: projected tracks may create `AudioBuffer`, `AudioStream`, and `AudioHandle` records, audio asset request plans, cleanup records, memory summaries, unload blockers, and `LoadAsset`/`StartTrack`/`UpdateTrack`/`StopTrack`/`ReleaseHandle` plans for a future `NativeAudioBackend`. `NullNativeAudioBackend` may record plans for tests. This is not playback support and must not be surfaced as `native-wgpu.audio@1` before a real decoder/mixer/device backend is implemented.
