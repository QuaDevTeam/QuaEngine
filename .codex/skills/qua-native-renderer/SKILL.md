...
- Current native resolved QSS projection fields are `background-color`, `background-image`, `border-color`, `border-radius`, `border-width`, `color`, `font-family`, `font-size`, `font-weight`, `line-height`, `text-align`, `object-fit`, `opacity`, and `z-index`; `background-image` is emitted as structured package-relative `asset(...)` metadata in `UiSurfaceResolvedStyle.backgroundImage`, never as browser `url(...)`, and `z-index` is emitted as `UiSurfaceNodeProjection.z_index` rather than stored in `UiSurfaceResolvedStyle`.
