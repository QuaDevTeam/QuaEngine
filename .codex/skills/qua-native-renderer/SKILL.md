...
- Quack native packaging should use `emitQuaProjectNativeTargetBundleManifest` when writing native `target-bundle-manifest.json`: it creates the manifest from the native artifact plan, validates it with `assertTargetBundleManifest(..., { expectedTarget: 'native' })`, and writes only after validation succeeds.
