...
- `@quajs/native-language-server`: standalone `.qui/.qss` LSP adapter and in-memory project index over `@quajs/native-ui-compiler`. It is an authoring tool only; it must not load Web/Cocos/native target core bootstrap plugins, ordinary game plugin arrays, or runtime QPK executable dependencies.
