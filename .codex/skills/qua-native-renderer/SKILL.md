...
- Rust `quajs_native_app` may use `QUA_NATIVE_RENDERER_SMOKE_FRAME` to read an already-resolved projection JSON fixture and submit it through `NativeRendererJsonFrameInput` plus `NullNativeRenderBackend` as a debug/CI smoke check. It should emit both a human-readable summary and a machine-readable `Qua native renderer smoke json: ...` line with missing-resource, fallback, declarative resource, memory, audio resource, and active audio track metrics. When `QUA_NATIVE_RENDERER_SMOKE_BUDGET` points to a JSON budget file, startup should fail if any smoke metric exceeds its configured max. This hook and budget gate must not load QUI/QSS authoring files, ordinary plugins, Web/Cocos/native target core bootstrap arrays, Runtime QPK executable dependencies, or any native payloads.
...
- `@quajs/native-language-server`: standalone `.qui/.qss` LSP adapter, in-memory project index, document-link helpers, component/class reference queries, LSP documentLink/references providers, and a package bin that defaults to stdio when no transport argument is supplied. It is an authoring tool only; it must not load Web/Cocos/native target core bootstrap plugins, ordinary game plugin arrays, or runtime QPK executable dependencies.
...
- Validate both `specifier` and `packageName` fields on target bundle references, plugin import references, and Runtime QPK executable/renderer references. A safe-looking `packageName` must not mask a Web, Cocos, or native target-core subentry in `specifier`, or vice versa.
