...
- `@quajs/engine-native` must hook engine runtime package unload notifications and call `releaseQuickJsPackageNamespaces(packageId)` when the host supports it, so successful QuickJS namespace handles do not outlive their owning Runtime QPK.
