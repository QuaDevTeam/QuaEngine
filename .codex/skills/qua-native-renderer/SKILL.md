...
- Native runtime package compatibility may be declared as `metadata.nativeRenderer` or target-scoped `metadata.renderers.native`; checks normalize `renderer`/`rendererPackage` to `packageName`, `version`/`rendererVersion` to `versionRange`, and `capabilityIds`/`optionalCapabilityIds` to capability requirement lists before activation. Required `assetKinds`, `qssFeatures`, and `quiComponents` must be checked against host renderer capabilities before activation. Native compatibility blocks must explicitly declare `nativeCode: false`; missing or truthy values are rejected before signature verification or script evaluation.
