...
For `demo/` narrative work, read `demo/.agents/README.md` and its linked world, character, mystery, timeline, relationship, and prologue documents first. The current project is **明天，请再一次呼唤我 / Call Me Again Tomorrow**, with 神代凛 and Mara; the previous story and media have been retired. The shared Web/native story plugin executes 14 `.qs` modules spanning the prologue, seven chapters, epilogue and three endings. `demo-story` owns grouped saved choices, stable step IDs, ending resolution and load/menu-return continuation. Chapter replay restores chapter-start saves, never later choices. The current text is a complete first draft, shorter than the final expansion budget. The user has authorized missing artwork and allowed either built-in image generation or Replicate CLI `openai/gpt-image-2.5-flare`, at most `medium`; the older image-generation pause is superseded. Only visually reviewed assets with real project files may be integrated. Speakerless narration uses third-person limited perspective centered on Rin; character dialogue retains each speaker's own perspective. Future-audio timestamps, knowledge, and relationship continuity must follow the archived story baseline.

### Narrative direction and insert illustrations

See `demo/.agents/narrative-direction-2026-09-12.md` for research, concrete revisions and validation, and `demo/.agents/narrative-direction-assets.json` for prompts, prediction IDs, rejected attempts and shipped image hashes.

- Revise scene goals, obstacles, information and emotional turns before polishing sentences. Write subtext through specific observable action, without appending an explanation of the character's growth to every gesture.
- Author movement at a change in attention, distance or intention. Do not apply the same bounce to every speaker. Preserve outfit-specific expression keys and start a timeline from the character's actual committed logical position.
- Use existing `AnimationTimeline`/`Key`, character enter/exit and background decorators; game-facing coordinates remain in the 1920×1080 logical stage. Commit final positions through engine projections so save/load and both targets agree. `CharacterExit` commits opacity as well as visibility: after a fade-out, pair the next `ShowCharacter` with `CharacterEnter` to restore opacity. A visible flag alone does not prove that a sprite is painted.
- Full-screen object inserts use `SetBackground('inserts/...')` and explicitly hide the cast. Restore the correct location, costume, expression and position when returning to people. A speaker in the text need not be visible during a close-up, telephone call or recording.
- Select the insert's exact entry and exit in the prose: do not reveal both umbrellas before the comparison, keep the intact pudding shot before eating, and end the hands insert before an embrace. Do not expose death reports or imagined fire scenes before the protagonist knows them.
- Real location changes need an authored image. Reserve `ClearBackground` for intentional darkness with a defined dramatic purpose, not missing asset placeholders.
- Visually inspect objects against clues and inspect generated images for baked-in UI. Confirm actual QPK image bytes, cast visibility, committed motion and fresh-page insert save/load in `demo/scripts/story-smoke.mjs`; compilation alone is not visual acceptance.
...
## Demo dialogue continuity review (2026-09-13)

Read `demo/.agents/dialogue-coherence-2026-09-12.md` for the latest all-14-module pass. Revise connected exchanges, not isolated synonyms: identify what each question asks, which person performs the next action, and what evidence supports the answer. Preserve intentional avoidance during Rin’s warning concealment; remove accidental non sequiturs, invented prior dialogue, premature conclusions and repetitive misunderstandings of ordinary objects. Track agreed meeting times, film/loan follow-through, clothing, physical handoffs and source versus receipt timestamps. A screenshot or file count cannot prove audio completeness; a current measurement cannot become an earlier unperformed measurement. Re-read new wording on both sides before validation.
