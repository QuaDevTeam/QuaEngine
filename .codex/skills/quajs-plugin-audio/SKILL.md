...
The demo native artifact and `dev:native`/native smoke commands enable `native-audio-rodio` and project `demo-native-bgm` from the existing QPK `bgm/blackout-cold-open.m4a` asset. `@quajs/engine-native` maps plugin BGM/voice/SFX/ambient tracks to the Quack `audio` asset kind by default while retaining the track kind separately for playback policy and resource ids. Demo frame validation must prove the engine-owned BGM projection, and native smoke must prove the product backend receives load/start commands, reaches a non-zero peak active-track count, and returns to zero active tracks after shutdown. Do not treat command-plan metrics alone as proof of a real audio backend when the feature is disabled.
