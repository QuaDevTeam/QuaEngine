...
import { getConfigPath, getProjectRoot, getTrashedProjectRoot } from '../src/lib/server/paths'
...
describe('local storage', () => {
  it('migrates a single-model config without changing behavior and persists independent model choices', async () => {
    tempHome = await mkdtemp(join(tmpdir(), 'novel-writer-config-'))
    process.env.NOVEL_WRITER_HOME = tempHome
    await writeFile(getConfigPath(), JSON.stringify({ deepSeekApiKey: 'local-test-key', deepSeekModel: 'deepseek-v4-flash', defaultReasoningEffort: 'max' }))
    expect(await readConfig()).toMatchObject({ codeModel: 'deepseek-v4-flash', codeReasoningEffort: 'max' })
    await updateConfig({ deepSeekModel: 'deepseek-v4-pro', defaultReasoningEffort: 'high' })
    expect(await readConfig()).toMatchObject({ deepSeekModel: 'deepseek-v4-pro', codeModel: 'deepseek-v4-flash', codeReasoningEffort: 'max' })
    const publicConfig = toPublicConfig(await readConfig())
    expect(publicConfig.hasDeepSeekApiKey).toBe(true)
    expect(JSON.stringify(publicConfig)).not.toContain('local-test-key')
...
describe('agent references and planner messages', () => {
  ...
  it('returns and records a fallback planner reply when DeepSeek is not configured', async () => {
    ...
    const pendingReply = appendUserMessage(project.id, '我希望故事更偏悬疑。')
    expect(isProjectRunning(project.id)).toBe(true)
    expect(activeWorkflowCount()).toBe(1)
    await expect(appendUserMessage(project.id, '重复请求')).rejects.toThrow('正在处理请求')
    await expect(startWorkflow(project.id)).rejects.toThrow('already running')
    await settleWorkflows()
    expect(isProjectRunning(project.id)).toBe(false)
