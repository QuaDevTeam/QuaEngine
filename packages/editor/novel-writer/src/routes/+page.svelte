<script lang="ts">
  import { createEditorWriting } from '$lib/client/editor-writing.svelte'
  import { connectDesktop } from '$lib/client/desktop.svelte'
  import { onDestroy, onMount, untrack } from 'svelte'
  import EditorIntegration from '$components/workspace/EditorIntegration.svelte'
  ...
  let { data }: { data: NovelWriterPageData } = $props()

  const workspace = createWorkspaceController(untrack(() => data))

  const writing = untrack(() => data.embedded) ? createEditorWriting(workspace) : undefined
  let integration = $state<EditorIntegration>()

  $effect(() => workspace.syncSelectedArtifactDraft())

  onMount(() => {
    workspace.init()
    const disconnect = connectDesktop(workspace)
    const unsubscribe = writing?.init()
...
<div class="app-shell" class:desktop-shell={data.embedded}>
  <AppTopbar onOpenSettings={workspace.openConfigDialog} {writing}
    onOpenSource={() => integration?.openSource()} onOpenDraft={() => integration?.openDraft()} onOpenContext={() => integration?.openContext()} />

  <div class="workspace">
    <ProjectSidebar
      projects={workspace.projects.filter(project => !workspace.editorRoot || !project.editorRoot || project.editorRoot === workspace.editorRoot)}
    ...
    <MainWorkspace
      onWriteProject={writing ? () => integration?.openArtifact() : undefined}
  ...
  {#if writing}<EditorIntegration bind:this={integration} {workspace} {writing} />{/if}
</div>
...
<SettingsDialog
  ...
  bind:codeModel={workspace.codeModel}
  bind:codeReasoningEffort={workspace.codeReasoningEffort}
