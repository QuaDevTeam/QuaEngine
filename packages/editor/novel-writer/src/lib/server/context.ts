...
export function formatPinnedProjectCanon(project: NovelProject): string {
  ...
  if (project.seed) {
    blocks.push(
      ...
      project.seed.allowExpertChanges
        ...
        : '允许专家修改设定：关闭。用户预设的所有内容和细节都是固化 canon，除用户明确写下的对应修改指示外，专家只能丰富、补充、细化，不得改写、删除、反转或矛盾化；评审专家必须检查这一项。',
    )
  }

  if (project.seed?.modificationInstructions) {
    blocks.push(
      '',
      '## 用户高级输入修改指示（必须先应用到预设内容）',
      project.seed.modificationInstructions,
    )
  }

  if (project.seed?.worldbuildingModificationInstructions) {
    blocks.push(
      '',
      '## 用户世界观修改指示（必须先应用到预设世界观）',
      project.seed.worldbuildingModificationInstructions,
    )
  }
  ...
  if (project.seed?.charactersModificationInstructions) {
    blocks.push(
      '',
      '## 用户角色设定修改指示（必须先应用到预设角色信息）',
      project.seed.charactersModificationInstructions,
    )
  }
  if (project.seed?.characters) {
    blocks.push('', `## 用户预设角色信息（${seedHeadingPolicy}）`, project.seed.characters)
  }
  if (project.seed?.outlineModificationInstructions) {
    blocks.push(
      '',
      '## 用户大纲修改指示（必须先应用到预设大纲）',
      project.seed.outlineModificationInstructions,
    )
  }
