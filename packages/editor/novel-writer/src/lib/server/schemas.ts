...
export const configUpdateSchema = z.object({
  ...
  codeModel: z.enum(['deepseek-v4-pro', 'deepseek-v4-flash']).optional(),
  codeReasoningEffort: z.enum(['high', 'max']).optional(),
...
export const createProjectSchema = z.object({
  ...
  editorRoot: z.string().min(1).max(4096).optional(),
...
}).refine(value => !value.editorSource || value.editorSource.root === value.editorRoot, '源稿与当前项目不匹配。')
