...
import { writeEditorSource } from './editor-source'
import type {
  ArtifactRef,
  NovelProject,
  NovelWriterConfig,
...
import { getAppHome, getConfigPath, getProjectRoot, getProjectsRoot, getTrashedProjectRoot, getTrashRoot } from './paths'
...
export function toPublicConfig(config: NovelWriterConfig): PublicNovelWriterConfig {
  return {
    ...
    codeModel: config.codeModel ?? config.deepSeekModel,
    codeReasoningEffort: config.codeReasoningEffort ?? config.defaultReasoningEffort,
...
export async function writeConfig(config: NovelWriterConfig): Promise<NovelWriterConfig> {
  ...
  await writeAtomic(getConfigPath(), `${JSON.stringify(normalized, null, 2)}\n`)
  return normalized
}

export async function updateConfig(update: Partial<NovelWriterConfig>): Promise<NovelWriterConfig> {
  const current = await readConfig()
  return writeConfig({ ...current, codeModel: current.codeModel ?? current.deepSeekModel, codeReasoningEffort: current.codeReasoningEffort ?? current.defaultReasoningEffort, ...update })
}
...
export async function createProject(input: {
  ...
  const id = `${slugify(input.title)}-${randomUUID()}`
  ...
  if (input.editorSource) await writeEditorSource(id, input.editorRoot, input.editorSource)
...
async function writeProjectToRoot(root: string, project: NovelProject): Promise<void> {
  ...
  await writeAtomic(join(root, 'project.json'), `${JSON.stringify(project, null, 2)}\n`)
}
...
export async function writeArtifact(artifact: ArtifactRef): Promise<ArtifactRef> {
  ...
  const safeId = artifact.id.replace(/[^\w-]/g, '_')
  const stamped = { ...artifact, updatedAt: new Date().toISOString() }
  await writeAtomic(join(root, `${safeId}.md`), stamped.markdown)
  await writeAtomic(join(root, `${safeId}.json`), `${JSON.stringify(stamped, null, 2)}\n`)
  return stamped
}
...
function normalizeConfig(config: NovelWriterConfig): NovelWriterConfig {
  return {
    ...
    codeModel: config.codeModel || config.deepSeekModel || defaultConfig.deepSeekModel,
    codeReasoningEffort: config.codeReasoningEffort || config.defaultReasoningEffort || defaultConfig.defaultReasoningEffort,
