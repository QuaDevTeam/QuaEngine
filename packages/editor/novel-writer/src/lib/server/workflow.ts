...
import type { DeepSeekMessage, DeepSeekRunResult, RegisteredDeepSeekTool } from './deepseek'
import type { TavilySearchInput } from './tavily'
...
export const activeWorkflowCount = (): number => activeRuns.size + activeRequests.size
export async function settleWorkflows(): Promise<void> {
  // An approval request can start regeneration while shutdown is waiting.
  while (activeRuns.size || activeRequests.size)
    await Promise.allSettled([...activeRuns.values(), ...activeRequests.values()])
}

function trackProjectRequest<T>(projectId: string, operation: () => Promise<T>): Promise<T> {
  const result = Promise.resolve().then(operation).finally(() => { activeRequests.delete(projectId) })
  activeRequests.set(projectId, result)
  return result
}
...
function extractChapterTitles(markdown: string): string[] {
  const pattern = /^##\s*第\s*[一二三四五六七八九十\d]+\s*章[\s：:]/m
  return markdown
    .split('\n')
...
/** Invalidate dependent work without losing unrelated chapter progress. */
function invalidateArtifact(checkpoint: Awaited<ReturnType<typeof readCheckpoint>>, artifact: ArtifactRef, includeSelf: boolean) {
  const affected = new Set(stagesAfter(artifact.stage))
  if (includeSelf) affected.add(artifact.stage)
  const chapter = chapterStages.includes(artifact.stage) && artifact.chapterIndex !== undefined
  const completedStages = (checkpoint?.completedStages ?? []).filter(stage => !affected.has(stage) && !(chapter && chapterStages.includes(stage)))
  const completedChapterStages = (checkpoint?.completedChapterStages ?? []).filter(item =>
    !affected.has(item.stage) || (chapter && item.chapterIndex !== artifact.chapterIndex),
  )
  return { ...(checkpoint ?? emptyCheckpoint(artifact.projectId)), completedStages, completedChapterStages }
}

export function isProjectRunning(projectId: string): boolean {
  return activeRuns.has(projectId) || activeRequests.has(projectId)
}
...
export async function recordApproval(projectId: string, input: ApprovalInput): Promise<void> {
  if (isProjectRunning(projectId)) throw new Error(`Project is already running: ${projectId}`)
  return trackProjectRequest(projectId, () => applyApproval(projectId, input))
}

async function applyApproval(projectId: string, input: ApprovalInput): Promise<void> {
  ...
  if (input.action === 'regenerate') {
    ...
    await mutateCheckpoint(projectId, checkpoint => ({
      ...invalidateArtifact(checkpoint, artifact, true),
      runId,
  ...
  if (input.action === 'approve') {
    await mutateCheckpoint(projectId, (checkpoint) => {
      const next = invalidateArtifact(checkpoint, artifact, false)
      const chapter = chapterStages.includes(artifact.stage) && artifact.chapterIndex !== undefined
      return {
        ...next,
        currentStage: artifact.stage,
        completedStages: chapter ? next.completedStages : mergeCompletedStages(next.completedStages, [artifact.stage]),
        completedChapterStages: chapter
          ? mergeCompletedChapterStages(next.completedChapterStages, [{ stage: artifact.stage, chapterIndex: artifact.chapterIndex! }])
          : next.completedChapterStages,
        awaitingApprovalArtifactId: undefined,
        updatedAt: new Date().toISOString(),
      }
    })
...
async function runRegeneration(projectId: string, runId: string, replacedArtifact: ArtifactRef, note?: string): Promise<void> {
  try {
    const artifact = await runStage(projectId, runId, replacedArtifact.stage, 'needs_review', 0, {
      ...
      chapterIndex: replacedArtifact.chapterIndex,
    })
    const project = await readProject(projectId)
    project.status = 'awaiting_review'
...
export async function appendUserMessage(projectId: string, content: string): Promise<{ reply: string }> {
  if (activeRequests.has(projectId)) throw new Error('当前任务正在处理请求，请稍后再试。')
  return trackProjectRequest(projectId, () => recordUserMessage(projectId, content))
}

async function recordUserMessage(projectId: string, content: string): Promise<{ reply: string }> {
  const project = await readProject(projectId)
...
async function runChapterLoop(
  ...
  if (requestedChapterIndex !== undefined && (requestedChapterIndex < 0 || requestedChapterIndex >= totalChapters))
    throw new Error('请求的章节不在当前大纲中。')
  await mutateCheckpoint(projectId, cp => ({ ...(cp ?? emptyCheckpoint(projectId)), totalChapters }))
  ...
  if (mode === 'yolo') {
    ...
    for (let idx = 0; idx < totalChapters; idx += 1) {
      ...
      if (!isChapterStageDone('scene_writing', idx)) {
        await runStage(projectId, runId, 'scene_writing', 'draft', 0, { chapterIndex: idx, chapterTitle: title, feedback })
        await markChapterStageComplete(projectId, runId, 'scene_writing', idx, totalChapters)
      }
      await runChapterPostStages(projectId, runId, idx, title, completedChapterStages, feedback)
    ...
    if (postPendingIdx !== undefined) {
      ...
      await runChapterPostStages(projectId, runId, postPendingIdx, title, completedChapterStages, feedback)
    ...
    const targetIdx = requestedChapterIndex ?? (() => {
      for (let i = 0; i < totalChapters; i += 1) {
        if (!isChapterStageDone('scene_writing', i))
          return i
      }
      return undefined
    })()
    ...
    const artifact = await runStage(projectId, runId, 'scene_writing', 'needs_review', 0, { chapterIndex: targetIdx, chapterTitle: title, feedback })
...
async function runChapterPostStages(
  ...
  if (!isDone('chapter_editing')) {
    await runStage(projectId, runId, 'chapter_editing', 'draft', 0, { chapterIndex, chapterTitle, feedback })
    await mutateCheckpoint(projectId, cp => ({
      ...(cp ?? emptyCheckpoint(projectId)),
  ...
  if (!isDone('chapter_supervision')) {
    await runStage(projectId, runId, 'chapter_supervision', 'draft', 0, { chapterIndex, chapterTitle, feedback })
    await mutateCheckpoint(projectId, cp => ({
      ...(cp ?? emptyCheckpoint(projectId)),
...
async function runProjectRevision(projectId: string, runId: string, revision: ProjectInputRevision): Promise<void> {
  ...
  try {
    ...
    await runYoloStages(project, runId, revision.affectedStages.filter(stage => !chapterStages.includes(stage) && !postChapterStages.includes(stage)), revision.feedback)
    if (revision.affectedStages.some(stage => chapterStages.includes(stage)))
      await runChapterLoop(project, runId, 'yolo', undefined, await readCheckpoint(projectId), revision.feedback)
    await runYoloStages(project, runId, revision.affectedStages.filter(stage => postChapterStages.includes(stage)), revision.feedback)
...
async function acceptPendingReviewForYolo(projectId: string, runId: string): Promise<void> {
  ...
  await mutateCheckpoint(projectId, (current) => {
    ...
    if (!chapterStages.includes(artifact.stage) || artifact.chapterIndex === undefined) {
    ...
    return {
      ...
      completedChapterStages: chapterStages.includes(artifact.stage) && artifact.chapterIndex !== undefined
        ? mergeCompletedChapterStages(current?.completedChapterStages, [{ stage: artifact.stage, chapterIndex: artifact.chapterIndex }])
