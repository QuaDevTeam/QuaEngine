...
const FORBIDDEN_ASSETS_NATIVE_RUNTIME_IMPORTS = [
  '@quajs/assets-cocos',
  '@quajs/assets-web',
  '@quajs/cocos-host',
  '@quajs/engine-native',
...
describe('@quajs/assets-native target isolation', () => {
  it('does not import Web, Cocos, engine-native, or store runtime adapters', () => {
    const roots = [
...
function collectForbiddenRuntimeImportViolations(roots: readonly string[]): string[] {
  return roots.flatMap(root => sourceFiles(root)).flatMap((filePath) => {
    ...
    return importSpecifiers(source)
      .filter(specifier => FORBIDDEN_ASSETS_NATIVE_RUNTIME_IMPORTS.some(packageName =>
