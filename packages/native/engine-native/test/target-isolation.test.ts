...
const FORBIDDEN_ENGINE_NATIVE_RUNTIME_IMPORTS = [
  '@quajs/assets-cocos',
  '@quajs/assets-native',
  '@quajs/assets-web',
  '@quajs/cocos-host',
...
describe('@quajs/engine-native target isolation', () => {
  it('does not import Web, Cocos, or platform storage/asset runtime adapters', () => {
    const roots = [
...
function collectForbiddenRuntimeImportViolations(roots: readonly string[]): string[] {
  return roots.flatMap(root => sourceFiles(root)).flatMap((filePath) => {
    ...
    return importSpecifiers(source)
      .filter(specifier => FORBIDDEN_ENGINE_NATIVE_RUNTIME_IMPORTS.some(packageName =>
