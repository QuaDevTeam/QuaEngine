...
#[test]
fn bench_smoke_reuses_box_shadow_lab_submission_under_frame_budget() {
    let mut renderer = NativeRenderer::new(WgpuNativeRenderBackend::new(
        WgpuNativeRenderBackendConfig::default(),
    ));
    renderer
        .prepare_and_render_json_str(BOX_SHADOW_LAB_FRAME)
        .expect("initial box shadow lab frame should render");

    let start = Instant::now();
    for _ in 0..WGPU_STABLE_SUBMIT_ITERATIONS {
        renderer
            .render_frame()
            .expect("stable box shadow lab frame should submit");
    }
    let elapsed_ms = start.elapsed().as_secs_f64() * 1_000.0;
    let average_frame_ms = elapsed_ms / WGPU_STABLE_SUBMIT_ITERATIONS as f64;
    let projected_fps = 1_000.0 / average_frame_ms.max(f64::EPSILON);
    println!(
        "{{\"bench\":\"native.wgpu.box_shadow.stable_submit\",\"iterations\":{},\"commands\":{},\"planReuses\":{},\"elapsedMs\":{:.3},\"averageFrameMs\":{:.3},\"projectedFps\":{:.1}}}",
        WGPU_STABLE_SUBMIT_ITERATIONS,
        renderer.metrics().frame.command_count,
        renderer.backend().stable_plan_reuse_count(),
        elapsed_ms,
    ...
    assert!(
        average_frame_ms < 16.7,
        "stable box shadow submission exceeded the 60 FPS CPU frame budget: {average_frame_ms:.3}ms"
    );
}
