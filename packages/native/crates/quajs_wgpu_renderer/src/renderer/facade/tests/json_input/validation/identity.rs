...
#[test]
fn json_frame_identity_validation_rejects_malformed_surface_node_bounds() {
    let mut renderer = NativeRenderer::new(NullNativeRenderBackend::new());

    let error = renderer
        .prepare_frame_json_str(json_frame_with_malformed_surface_node_bounds_input())
        .unwrap_err();
    ...
    match error {
        NativeRendererJsonFrameError::Validation(validation) => {
            ...
            assert!(validation.reason.contains("resolved projection JSON"));
        }
        other => panic!("expected malformed surface node bounds validation error, got {other:?}"),
    }
    assert_eq!(renderer.state().revision(), 0);
    assert!(renderer.state().frame().is_none());
}
...
#[test]
fn json_frame_identity_validation_rejects_malformed_surface_node_children() {
    let mut renderer = NativeRenderer::new(NullNativeRenderBackend::new());

    let error = renderer
        .prepare_frame_json_str(json_frame_with_malformed_surface_node_children_input())
        .unwrap_err();

    match error {
        NativeRendererJsonFrameError::Validation(validation) => {
            assert_eq!(validation.path, "view.ui.overlays[0].surface.root.children");
            assert_eq!(validation.asset_name, "Column");
            assert!(validation.reason.contains("must be an array"));
            assert!(validation.reason.contains("resolved projection JSON"));
        }
        other => panic!("expected malformed surface children validation error, got {other:?}"),
    }
    assert_eq!(renderer.state().revision(), 0);
    assert!(renderer.state().frame().is_none());
}
...
#[test]
fn json_frame_identity_validation_rejects_malformed_surface_node_style() {
    let mut renderer = NativeRenderer::new(NullNativeRenderBackend::new());

    let error = renderer
        .prepare_frame_json_str(json_frame_with_malformed_surface_node_style_input())
        .unwrap_err();

    match error {
        NativeRendererJsonFrameError::Validation(validation) => {
            assert_eq!(validation.path, "view.ui.overlays[0].surface.root.style");
            assert_eq!(validation.asset_name, "");
            assert!(validation.reason.contains("must be an object"));
            assert!(validation.reason.contains("surface node styles"));
        }
        other => panic!("expected malformed surface node style validation error, got {other:?}"),
    }
    assert_eq!(renderer.state().revision(), 0);
    assert!(renderer.state().frame().is_none());
}
