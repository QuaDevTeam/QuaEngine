...
#[test]
fn emits_underline_decoration_for_text_placeholder_geometry() {
    ...
    assert!(pass.vertices[4].position[1] > pass.vertices[0].position[1]);
    assert_eq!(pass.vertices[4].color, [1.0, 1.0, 1.0, 1.0]);
    assert!(pass.draw_calls[0].physical_bounds.height > 4);
}

#[test]
fn emits_line_through_decoration_for_text_placeholder_geometry() {
    let mut style = text_style(20.0, TextAlign::Left, EdgeInsetsDrawParam::default());
    ...
    assert_eq!(pass.index_count, 12);
    assert!(pass.vertices[4].position[1] > pass.vertices[0].position[1]);
    assert!(pass.vertices[4].position[1] < pass.vertices[2].position[1]);
}
