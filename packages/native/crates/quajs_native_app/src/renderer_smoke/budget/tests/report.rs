...
fn reports_all_budget_violations() {
    ...
    let report = NativeRendererSmokeBudgetReport::check(
        NativeRendererSmokeBudget {
            ...
            max_audio_backend_applied_plans: Some(1),
            max_audio_backend_applied_commands: Some(2),
            max_audio_backend_active_tracks: Some(0),
            max_backend_draws: Some(3),
            max_backend_passes: Some(1),
            max_backend_commands: Some(7),
            max_backend_pipeline_binds: Some(1),
            max_backend_clip_sets: Some(0),
            max_backend_clip_clears: Some(0),
            max_backend_max_clip_depth: Some(0),
            max_backend_skipped_draws: Some(0),
            max_backend_resource_binds: Some(1),
            max_backend_bound_resource_references: Some(2),
            max_backend_unique_resources: Some(1),
            max_backend_missing_resource_references: Some(0),
            max_backend_unique_missing_resources: Some(0),
    ...
    assert_eq!(
        ...
        vec![
            ...
            "audioBackend.appliedPlanCount=2 exceeded max 1",
            "audioBackend.appliedCommandCount=3 exceeded max 2",
            "audioBackend.activeTrackCount=1 exceeded max 0",
            "backend.drawCount=4 exceeded max 3",
            "backend.passCount=2 exceeded max 1",
            "backend.commandCount=8 exceeded max 7",
            "backend.pipelineBindCount=2 exceeded max 1",
            "backend.clipSetCount=1 exceeded max 0",
            "backend.clipClearCount=1 exceeded max 0",
            "backend.maxClipDepth=1 exceeded max 0",
            ...
            "backend.uniqueResourceCount=2 exceeded max 1",
            "backend.missingResourceReferenceCount=1 exceeded max 0",
            "backend.uniqueMissingResourceCount=1 exceeded max 0",
