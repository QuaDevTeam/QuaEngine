...
export class SceneManager {
  ...
  async showCharacter(
    ...
    logger.debug(`Showing character: ${characterId}`)

    await this.engine['pipeline'].emit(LogicToRenderEvents.CHARACTER_SHOW, {
      characterId,
      assetName,
      position,
  ...
  /**
   * Hide character from scene
   */
  async hideCharacter(
    characterId: string,
    animation?: { type: string; duration: number }
  ): Promise<void> {
    logger.debug(`Hiding character: ${characterId}`)

    await this.engine['pipeline'].emit(LogicToRenderEvents.CHARACTER_HIDE, {
      characterId,
      animation
    })
  }
  ...
  async moveCharacter(
    ...
    logger.debug(`Moving character: ${characterId}`)

    await this.engine['pipeline'].emit(LogicToRenderEvents.CHARACTER_MOVE, {
      characterId,
      position,
      duration
    })
   ...
   */
  async setCharacterExpression(
    characterId: string,
    expression: string
  ): Promise<void> {
    logger.debug(`Setting character expression: ${characterId} -> ${expression}`)

    await this.engine['pipeline'].emit(LogicToRenderEvents.CHARACTER_EXPRESSION, {
      characterId,
      expression
    })
  }
