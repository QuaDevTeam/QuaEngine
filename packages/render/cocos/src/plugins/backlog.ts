...
function renderBacklogLayer(context: CocosRendererPluginContext, requestedPage: number): void {
  ...
  context.cocos.host.nodes.setNodeTransform(subtitle, { x: safeArea.x + 28, y: safeArea.y + 62, width: safeArea.width - 180, height: 32, zIndex: 1 })
  ...
  if (projection.entries.length === 0) {
    ...
    context.cocos.host.nodes.setNodeTransform(empty, { x: safeArea.x + 28, y: safeArea.y + 110, width: safeArea.width - 56, height: 56, zIndex: 10 })
    context.cocos.host.nodes.setNodeMetadata?.(empty, { plugin: 'backlog', empty: true })
  }
  page.items.forEach((entry, index) => renderBacklogEntry(context, panel, entry, page.page * 8 + index, index, safeArea.x + 28, safeArea.y + 110, safeArea.width - 56))
  renderBacklogButton(context, panel, 'backlog:page:prev', 'Prev', {
    x: safeArea.x + 28,
    y: safeArea.y + 110 + 8 * 84,
    width: 96,
    height: 44,
  ...
  context.cocos.host.nodes.setNodeTransform(pageLabel, { x: safeArea.x + 244, y: safeArea.y + 110 + 8 * 84, width: 96, height: 44, zIndex: 20 })
}
...
function renderBacklogEntry(
  ...
  context.cocos.host.nodes.setNodeTransform(meta, { x: x + 12, y: y + 6, width: 220, height: 20, zIndex: index + 3 })
  ...
  context.cocos.host.nodes.setNodeTransform(content, { x: x + 12, y: y + 28, width: Math.max(0, (entry.voice ? width - 116 : width) - 24), height: 30, zIndex: index + 3 })
  if (entry.tags?.length) {
    ...
    context.cocos.host.nodes.setNodeTransform(tags, { x: x + 240, y: y + 6, width: Math.max(0, width - 360), height: 20, zIndex: index + 3 })
  }
  if (entry.voice) {
