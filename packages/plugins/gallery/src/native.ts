...
function createGalleryRoot(
  ...
  const panel = { x: available.x + (available.width - width) / 2, y: available.y + (available.height - height) / 2, width, height }
      ...
      node('gallery-panel', 'Panel', panel, {
        ...
        children: [
          ...
          node('gallery-count-badge', 'Panel', {
            x: panel.x + panel.width - edge - 182,
            y: panel.y + edge + 5,
            width: 130,
            height: 34,
          }, { provenance, style: { backgroundColor: 'rgba(229,193,111,0.18)', borderColor: 'rgba(229,193,111,0.34)', borderWidth: 1 } }),
          ...
          ...(!grid
            ...
            : []),
          ...(grid
            ? [node('gallery-entry-pane', 'Panel', {
                x: panel.x + edge + 1,
                y: panel.y + 87,
                width: panel.width - edge * 2 - 2,
                height: panel.height - 87 - edge - 1,
              }, { provenance, style: { backgroundColor: 'rgba(6,7,11,0.8)', borderColor: 'rgba(245,226,190,0.16)', borderWidth: 1 } })]
            : []),
          ...(grid
            ? [node('gallery-entries-label', 'Text', {
                x: panel.x + edge + 12,
                y: panel.y + 96,
                width: 200,
                height: 16,
  ...
  return node('gallery-lightbox', 'Layer', stage(context), {
    ...
    children: [
      node('gallery-lightbox-backdrop', 'Backdrop', stage(context), {
        provenance,
        intent: uiIntent(ACTIONS.closePreview),
        style: { backgroundColor: 'rgba(0,0,0,0.78)' },
      }),
      node('gallery-lightbox-media', image ? 'Image' : 'Text', bounds, {
        image,
        text: image ? undefined : entry ? previewText(content, entry) : '',
        provenance,
        style: { objectFit: 'contain', color: '#fff8ea', fontSize: 20, whiteSpace: 'pre-wrap' },
      }),
      node('gallery-lightbox-close', 'Button', { x: bounds.x + bounds.width - 66, y: bounds.y + 22, width: 42, height: 42 }, {
        text: '×',
        intent: uiIntent(ACTIONS.closePreview),
        provenance,
        style: closeButtonStyle(),
      ...
      node('gallery-lightbox-caption-panel', 'Panel', {
        x: context.logicalWidth / 2 - captionWidth / 2,
        y: bounds.y + bounds.height - 66,
        width: captionWidth,
        height: 36,
      }, { provenance, style: { backgroundColor: 'rgba(229,193,111,0.22)', borderColor: 'rgba(245,226,190,0.22)', borderWidth: 1 }, children: [
  ...
  return node(`gallery-entry-${index}`, 'Button', bounds, {
    ...
    children: [
      node(`gallery-entry-${index}-preview-background`, 'Panel', { ...bounds, height: previewHeight }, {
        provenance,
        style: { backgroundColor: 'rgba(3,4,7,0.58)' },
      }),
      ...
      node(`gallery-entry-${index}-title`, 'Text', {
        x: bounds.x + 11,
        y: bounds.y + previewHeight + 5,
        width: bounds.width - 22,
        height: 28,
      }, { text: entry.title, provenance, style: { color: '#fff8ea', fontSize: 14, fontWeight: 700, textOverflow: 'ellipsis' } }),
      node(`gallery-entry-${index}-summary`, 'Text', {
        x: bounds.x + 11,
        y: bounds.y + previewHeight + 28,
        width: bounds.width - 22,
        height: 30,
      }, { text: entry.summary || '', provenance, style: { color: 'rgba(247,242,234,0.56)', fontSize: 12, textOverflow: 'ellipsis' } }),
      node(`gallery-entry-${index}-badge`, 'Panel', {
        x: bounds.x + 11,
        y: bounds.y + previewHeight + 59,
        width: 60,
        height: 20,
      }, { provenance, style: { borderColor: 'rgba(245,226,190,0.18)', borderWidth: 1 } }),
      node(`gallery-entry-${index}-state`, 'Text', {
        x: bounds.x + 11,
        y: bounds.y + previewHeight + 61,
        width: 60,
        height: 18,
      }, { text: entry.unlocked ? (entry.tags || []).join(' · ').toUpperCase() : 'LOCKED', provenance, style: { color: '#ffe3a0', fontSize: 10, letterSpacing: 1.2 } }),
    ],
  })
}
  ...
  return node('gallery-preview', 'Panel', bounds, {
    ...
    children: [
      ...
      node('gallery-content-scroll', 'Scroll', {
        ...
        width: bounds.width,
        height: tabsHeight,
      }, { clipChildren: true, provenance, children: contentButtons.length > 1
