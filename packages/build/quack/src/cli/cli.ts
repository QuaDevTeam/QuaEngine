...
program
  ...
  .option('--project-config <path>', 'Qua project configuration file path')
  .option('-v, --verbose', 'Verbose output')
  .action(async (options) => {
    try {
...
const project = program
  .command('project')
  .description('QuaEngine project manifest helpers')

project
  .command('validate')
  .description('Validate qua.project.yaml, qua.project.yml, or qua.project.json')
  .option('-c, --config <path>', 'Qua project configuration file path')
  .option('-v, --verbose', 'Verbose output')
  .action(async (options) => {
    try {
      const { config } = await loadProjectConfigForCli({ config: options.config })
      console.log(`✅ Qua project config is valid: ${config.name} (${config.bundleId})`)
      if (options.verbose) {
        console.log(JSON.stringify(config, null, 2))
      }
    }
    catch (error) {
      console.error('❌ Qua project config validation failed:', getErrorMessage(error))
      if (options.verbose) {
        console.error(getErrorStack(error))
      }
      process.exit(1)
...
project
  .command('doctor')
  .description('Diagnose Qua project manifest targets, icons, and generated platform readiness')
  .option('-c, --config <path>', 'Qua project configuration file path')
  .option('-v, --verbose', 'Verbose output')
  .action(async (options) => {
    try {
      const { config, projectRoot } = await loadProjectConfigForCli({
        config: options.config,
        validateAssets: false,
      })
      const result = await doctorQuaProjectConfig(config, { cwd: projectRoot })
      console.log(`Qua project doctor: ${config.name} (${config.bundleId})`)
      for (const issue of result.issues) {
        console.log(`${formatDoctorSeverity(issue.severity)} [${issue.target}] ${issue.message}`)
        if (options.verbose && issue.filePath) {
          console.log(`  file: ${issue.filePath}`)
        }
      }
      if (!result.ok) {
        console.error('❌ Qua project doctor found errors.')
        process.exit(1)
      }
      const warningCount = result.issues.filter(issue => issue.severity === 'warning').length
      console.log(warningCount > 0
        ? `⚠️ Qua project doctor completed with ${warningCount} warning(s).`
        : '✅ Qua project doctor passed.')
    }
    catch (error) {
      console.error('❌ Qua project doctor failed:', getErrorMessage(error))
      if (options.verbose) {
        console.error(getErrorStack(error))
      }
      process.exit(1)
...
project
  .command('sync')
  .description('Sync generated project files for target platforms')
  .option('-c, --config <path>', 'Qua project configuration file path')
  .option('-t, --target <target>', 'Target platform to sync (web|cocos|all)', 'all')
  .option('--platform <platform>', 'Specific Cocos platform to sync')
  .option('-v, --verbose', 'Verbose output')
  .action(async (options) => {
    try {
      const { config, projectRoot } = await loadProjectConfigForCli({ config: options.config })
      if (options.target === 'web') {
        console.log('✅ Web project files are generated by the Vite plugin during build.')
        return
      }
      if (options.target !== 'all' && options.target !== 'cocos') {
        throw new Error(`Unsupported project sync target "${options.target}". Expected web, cocos, or all.`)
      }
      const result = await syncQuaProjectCocos(config, { cwd: projectRoot, platform: options.platform })
      console.log(`✅ Synced ${result.files.length} Cocos project file(s).`)
      for (const file of result.files) {
        console.log(`  ${file}`)
      }
    }
    catch (error) {
      console.error('❌ Qua project sync failed:', getErrorMessage(error))
      if (options.verbose) {
        console.error(getErrorStack(error))
      }
      process.exit(1)
...
project
  ...
  .action(async (options) => {
    try {
      ...
      const { config, configPath, projectRoot } = await loadProjectConfigForCli({ config: options.config })
      const workspaceManager = new WorkspaceManager(projectRoot)
      const workspaceConfig = await workspaceManager.loadConfig(
        options.workspaceConfig ? resolve(projectRoot, options.workspaceConfig) : undefined,
        {
      ...
      if (options.bundle) {
        const bundle = workspaceManager.getBundleDefinition(options.bundle)
        if (!bundle) {
          throw new Error(`Bundle "${options.bundle}" not found`)
        }
        bundlesToBuild = [bundle]
      }
      else if (!options.all) {
        throw new Error('Specify --bundle <name> or --all to build project bundles.')
      }

      const buildOrder = workspaceManager.getBundlesBuildOrder()
      const selectedBundles = buildOrder.filter(bundle => bundlesToBuild.includes(bundle))
      for (const bundle of selectedBundles) {
        console.log(`\n📦 Building bundle: ${bundle.displayName || bundle.name}`)
        const bundleConfig = workspaceManager.createBundleConfig(bundle.name, {
          verbose: options.verbose,
      ...
      const result = await syncQuaProjectCocos(config, { cwd: projectRoot, platform: options.platform })
      console.log(`✅ Project Cocos build files ready: ${result.buildConfigs.length} platform config(s).`)
    }
    catch (error) {
      console.error('❌ Qua project build failed:', getErrorMessage(error))
      if (options.verbose) {
        console.error(getErrorStack(error))
      }
      process.exit(1)
...
async function loadProjectConfigForCli(options: {
  config?: string
  validateAssets?: boolean
}) {
  const configPath = await findQuaProjectConfigFile({ configPath: options.config })
  if (!configPath) {
    throw new Error('No Qua project config file found. Expected qua.project.yaml, qua.project.yml, or qua.project.json.')
  }
  const projectRoot = dirname(configPath)
  const config = await loadQuaProjectConfig({
    configPath,
    cwd: projectRoot,
    validateAssets: options.validateAssets,
  })
...
function formatDoctorSeverity(severity: 'info' | 'warning' | 'error'): string {
  switch (severity) {
    case 'error':
      return '❌'
    case 'warning':
