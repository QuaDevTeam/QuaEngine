...
- **Official presets**: QuaEngine provides `landscape` and `portrait` layout presets. Landscape uses a `1920x1080` / `16:9` reference and adapts across the `16:10` to `16:9` interval. Portrait uses a mobile-first `1080x2340` / `9:19.5` reference and adapts across the `9:21` to `9:16` interval so common mobile devices fill the container without black bars.
- **Layout resolution formula**: Renderers resolve `activeAspect = clamp(containerWidth / containerHeight, layout.minAspectRatio, layout.maxAspectRatio)`, fit that ratio into the container, set `scale = viewportHeight / layout.height`, keep `logicalHeight = layout.height`, and derive `logicalWidth = viewportWidth / scale`. `layout.aspectRatio` is the preferred/reference ratio, not the fixed rendered ratio.
...
- **Native/non-Web renderers and native asset adapters are not implemented**. Current official platform adapters are Web, Node, Memory, Web store, Node store, and Web security; current official Web rendering stack is `@quajs/renderer-web` plus Vue, React, and Svelte adapters.
...
- The default presets are part of the project contract: `landscape` adapts from `16:10` to `16:9`; `portrait` adapts from `9:21` to `9:16` with a `1080x2340` / `9:19.5` mobile reference. Do not reintroduce fixed `16:9` landscape-only assumptions or use landscape coordinates for portrait.
- The active stage ratio is the container ratio clamped into the configured interval. Renderer code must derive viewport size, logical width, scale, safe area, and hit-test conversion from the same resolved layout so visuals, input, and animation agree on every device.
